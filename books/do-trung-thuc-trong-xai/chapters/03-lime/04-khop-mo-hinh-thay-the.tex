...
\mathcal{L}(f,g,\pi_x)=
\sum_{(z,z') \in \mathcal{Z}} \pi_x(z)\bigl(f(z)-g(z')\bigr)^2,
\qquad
\pi_x(z)=\exp\left(-\frac{D(x,z)^2}{\sigma^2}\right).
\end{equation}

Ở đây $\mathcal{Z}$ là tập các mẫu đã sinh. Một mẫu xa vẫn có thể xuất hiện
trong phép khớp, nhưng trọng số của nó nhỏ hơn. Cách chọn $D$ phụ thuộc miền:
bài báo nêu khoảng cách cosine cho văn bản và khoảng cách Euclid cho ảnh. Như
vậy, từ \emph{cục bộ} không chỉ là trực giác hình học. Nó được thực hiện bằng
...
viết lại quy trình này dưới dạng pseudo-code; nó mô tả thuật toán của bài báo,
không phải mã đã chạy trong cuốn sách.

\begin{algorithm}[htbp]
  \caption{Pseudo-code cho lời giải thích LIME thưa, theo Ribeiro, Singh và
  Guestrin~\autocite{p09lime}}
  \label{alg:ch03-lime}
  \begin{algorithmic}[1]
