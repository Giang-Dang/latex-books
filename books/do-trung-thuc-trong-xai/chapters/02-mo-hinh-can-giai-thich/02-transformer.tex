...
Từ sự phân biệt giữa đầu vào, trạng thái và đầu ra ở mục
\ref{sec:ch02-doi-tuong-can-giai-thich}, ta có thể nhìn transformer như cơ
...
\tn{cơ chế chú ý}{attention mechanism}, không dùng recurrence hay convolution;
cấu trúc encoder-decoder của nó ghép self-attention với các lớp feed-forward
theo từng vị trí~\autocite{p01attention}.

Trong self-attention, biểu diễn tại một vị trí tạo truy vấn, khóa và giá trị.
...
ngữ cảnh thay vì chỉ nhận trạng thái từ vị trí liền trước. Multi-head attention
thực hiện nhiều phép chiếu song song; mỗi head có thể học một kiểu quan hệ, dù
việc quan sát một head có trọng số lớn chưa chứng minh head ấy là nguyên nhân
của một token đầu ra.
...
\index{transformer|(}%
Các biểu diễn cũng thay đổi qua các lớp. Một token ở lớp đầu có thể giữ nhiều
thông tin bề mặt, trong khi biểu diễn ở lớp sau chịu ảnh hưởng của toàn bộ
ngữ cảnh mà kiến trúc cho phép nó truy cập. Khi một phương pháp hậu nghiệm ở
