\begin{tikzpicture}[
  ...
  dashedarrow/.style={-{Stealth[length=2mm]}, draw=black!55, thick, dashed}
]
  ...
  \node[box, below=9mm of path] (ig) {Tích gradient theo đường đi\\và phân bổ $F(x)-F(x_0)$};
  \draw[arrow] (base) -- (path);
  \draw[arrow] (path) -- (input);
  \draw[dashedarrow] (path) -- (ig);
