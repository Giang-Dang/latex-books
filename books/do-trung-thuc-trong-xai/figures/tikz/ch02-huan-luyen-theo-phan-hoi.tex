\begin{tikzpicture}[
  ...
  \node[box, below=8mm of base] (sft) {Fine-tune có giám sát};
  \node[data, right=11mm of sft] (rankings) {Xếp hạng của người gán nhãn};
  \node[box, below=8mm of sft] (reward) {Reward model};
  \node[box, below=8mm of reward] (rl) {Tối ưu bằng reinforcement learning};
  \node[box, below=8mm of rl] (chat) {Mô hình hội thoại};
