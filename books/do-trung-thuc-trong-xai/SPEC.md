...
- **The limitation-statement log for chapter 18.** Each Part IV drafting
  ...
  as having no standard, and named as the next step not taken. Unblocked chapter
...
- **The remaining init-written TOC lines have not been re-checked against their
  ...
  line's subject could have been read off the paper's title alone. That applies
  to chapters 13, 14, 15, 16 and 17, all still not-started. Chapter 13's line
  ("the algorithmic-information-theoretic ceiling on explainability, and what a
  bound does and does not say about practice") and chapter 15's ("concept
  leakage as the CBM analogue of unfaithfulness") are the two that most read
  like a claim inferred from a title, so they are the first to check. This is
  cheap to discharge, one abstract per chapter before any drafting, and it
  closes when the last of those five is drafted.
