...
- **Research:** the cliff is crossed. `research/` holds a note from chapters 08,
  09, 10 and 12 (decision 35), so the Numbers and Verbatim checks are
...
- **The limitation-statement log for chapter 18.** Each Part IV drafting
  ...
  `research/ch12-con-nguoi-vang-mat.md`. **Four of the seven Part IV chapters
  are now logged**, leaving 11, 13 and 14. The note for chapter 10 records the
  ...
  observation, that is a reading decision for that session. Unblocked chapter by
  chapter as Part IV drafts.
- **Chapter 12 was drafted before chapter 11, so its opener answers a paragraph
  that does not exist.** The Cohesion rule's second half says a chapter opens on
  the last paragraph of the chapter before it. Chapter 11 is a three-line stub,
  so chapter 12's opener was written against what the TOC line promises chapter
  11 will end on: the research metagame's loop, and the fact that no step in it
  eliminates a method because no instrument can rank them. That is a guess about
  a paragraph nobody has written. **Chapter 11's session owns the
  reconciliation** and has two ways to do it: end chapter 11 on the paragraph
  chapter 12 already answers, or rewrite chapter 12's opening two paragraphs to
  answer whatever it does end on. Either is fine; leaving both as they are is
  not, and this item closes when chapter 11 is drafted. General rule this
  exposes: drafting Part IV out of order costs exactly one opener, which is
  cheap, but only if the debt is written down when it is incurred.
