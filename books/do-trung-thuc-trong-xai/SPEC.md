...
Chapter 14's session, 2026-09-05, **closes Part IV**. It is the first chapter in
this book to read three corpus papers, and the first to find that its papers do
not stand in the relation its own table-of-contents line asserted. That is
decision 69 and a sixth kind of TOC failure: not a line promising more than the
papers contain, nor one promising less, but one whose clauses are each true of
some paper while the arrangement between them is false. One paper's object is
SHAP being attacked, the next paper's is SHAP doing the attacking, and the third
has no adversary in it at all. The chapter says so in its first section rather
than letting the chapter title carry a claim about the third paper.

Three other things moved at book level. The book cites a second paper from
outside the corpus (decision 71), and the trigger for doing so widened: decision
34's test was that no corpus paper *defines* the thing, and the test now also
covers a corpus paper that only *replicates* what a chapter argues from. A
symbol collision was settled by shape for the second time (decision 72), which
is worth recording with its ratio rather than on its own, because four of the
same chapter's five collisions were renamed the ordinary way and only one met
the test. And decision 73 extends the decision 40 / 62 / 62c family down one
level, from an argument to a reported statistic: paper 29's column headed `RMSE`
is a sum of squared deviations, shown by reproducing eleven of its twelve cells,
and because the two readings are monotone in one another the chapter prints the
defect and keeps the paper's conclusion. **That reconstruction was wrong the
first time**, on a cell transcribed from the neighbouring column, and the cold
audit is what found it; decision 73 records the rule that follows.

**The verified absence this chapter found is the strongest in the book**: the
word `faithful` occurs zero times in all three of its papers. Chapter 13's was
one paper's silence across 65 pages; this is an entire tier of the reading
ladder, three papers by three unconnected groups, none of them reaching for the
word. The 2026-09-05 build is 191 pages against the 174 of the reading-flow pass, no overfull boxes, no undefined references, `main.ilg` 0 warnings, prose
gate clean, and the rendered pages inspected, which is how two defects invisible
to every gate were caught: figure 14.1 had floated above the paragraph that
introduces it, and table 14.1's caption promised a column the tabular did not
have.

Chapter 15's session, 2026-09-06, **opens Part V**, and what it found is the
opposite of what fourteen chapters had led the book to expect. Part IV's charge
is that the field's faithfulness metrics are unvalidated instruments; chapter
15's first paper proposes two metrics and then validates them against a criterion
its authors did not choose, which is the first time this book has met that
(decision 79). The reason is worth more than the fact: in a concept bottleneck
model the ground truth arrives annotated with the dataset, so nobody has to
construct it, and that is a fourth route beside the three section 14.7 counted.
Section 15.4 spends as much room on the three things the validation does not
reach as on the validation, because the alternative is a section that reads as
praise.

Four other things moved. **The TOC line failed by naming less**, decision 59's
mode a second time and the seventh init-written line to fail: it named leakage
and missed the measurement of it, and its middle clause turns out to be a claim
the book makes and no paper of the chapter does (decision 74). **The book cites a
third paper from outside the corpus and decision 34's trigger widens a third
time** (decision 76): paper 31 states a fourth paper's finding backwards, and the
chapter cannot say so without reading that fourth paper. Reading it produced
decision 77, which extends the decision 40 / 62 / 62c / 73 family one level out,
from a source's own claim to a source's report of another source. **That row
also records a charge this session made and the cold audit made it withdraw**,
against paper 30, and the withdrawal is the more useful half: having found one
real inversion, the session wrote down a second, symmetrical one without
rechecking the citation list, which is decision 62c happening again on the same
page it is quoted. And the book's own
arithmetic on a printed table fires for the second time after decision 73:
paper 32's `+14.0%` steerability figure is the sum of two per-metric gains where
the other five figures in the same sentence are their mean, so the corrected
value is +7.0% and the abstract's headline drops from +14.5% to 12.2%.

**The running example reaches Part V**, which the 2026-09-05 reading-flow pass
had taken only to chapter 08. The function chapters 01 to 08 use, thresholded at
1, is the OR of its two features, and that is the illustrative task paper 30
uses, so section 15.2 computes the concepts-task baseline and the leakage ceiling
on it by hand. The 2026-09-06 build is 208 pages against the 191 of chapter 14,
no overfull boxes, no undefined references, `main.ilg` 0 warnings, prose gate
clean, and the rendered pages inspected, which is how two occurrences of a term
split across a source line were caught after a rename had missed them.

Chapter 16's session, 2026-09-06, is **the first in this book to read no new
paper**, and most of what it settled follows from that. Its four papers were each
close-read by an earlier chapter, so the work was re-reading them for one
question none of their chapters had asked, which is what each escape route costs.
**The table-of-contents line failed in a fourth way** (decision 80): not a noun
that over-promises, under-describes or mis-arranges, but an organising predicate
that one of its own papers spends a passage rejecting. The line said the three
routes come "each with its cost", and paper 24 prints `cost`, `expensive` and
`power` zero times and argues that the obstacle is not cost. So the re-check the
open items carry gains a fourth leg, read the line's verb, and the item is now
one chapter wide. **The chapter's own finding is that the three routes are not
parallel exits**, and each leg of it is a source's sentence while the assembly is
the book's: paper 21 takes route 1 because it judges route 3 unavailable, paper
18 states the evaluation gap as shared across all three attribution arms and
names no ground truth for components anywhere in 25 pages, and paper 24's route
answers a different question. Decision 82 records what a no-new-paper session is
worth, because chapter 18 is the other chapter of that kind: this one produced
three quantities no earlier chapter had printed and **five corrections to three
earlier chapters' research notes**, none of which had reached the manuscript.

**Two defects that had shipped past every gate were found and fixed, and neither
is in chapter 16.** Section 15.7 carried a literal carriage return where a
backslash belonged, so a cross-reference printed as the macro's name in words; it
is not an undefined reference, so no check can see it. And chapter 13's review
question used one of chapter 08's terms unglossed, hidden because the term
wrapped inside an indented `cauhoids` block. Tracing the second one through
`check-chapter.ps1` corrected a standing open item: the Gloss check loses a
wrapped multi-word term to the **continuation line's indentation**, not to the
line break, so it is systematically blind inside `tomtat` and `cauhoids` and
nowhere else. Measured book-wide, 134 wrapped uses are still visible and 21 are
not, 13 of those in summary files. The 2026-09-06 build is 221 pages against the
208 of chapter 15, no overfull boxes, no undefined references, `main.ilg` 0
warnings, prose gate clean, and the rendered pages inspected.
...
| 69 | **A TOC line can also fail by mis-stating how its papers relate, which is the sixth finding and a new kind** | The initialized line for chapter 14 read "adversarial fooling of explanations, SHAP as an attack surface, causal structure as a partial repair". Checked against all three papers, and against the abstracts first as the open item requires. Each clause is defensible about *some* paper and the relation between them is wrong. Clause 1 names a construction **no corpus paper contains**: paper 27 replicates Slack et al. and says so in its own abstract, so the line's first subject lives outside the corpus (decision 71). Clause 2 fits papers 27 and 28 in opposite directions: in 27 SHAP is attacked, in 28 SHAP is the attacker and an image classifier is the victim, and "attack surface" is only the first sense. Clause 3 holds for paper 29, the only clause to survive contact, but **paper 29 has no adversary in it anywhere**; its weakness is statistical and appears under ordinary use, so a chapter titled *Giải thích dưới tấn công* has to say that rather than let the title carry the claim. Decisions 24, 28, 47 and 56 are lines that promise more than the paper contains and decision 59 is a line that promises less; this one promises the right things about the wrong arrangement. **The general rule this adds: the re-check decision 56 asks for is not only clause by clause. Where a line names several papers, check whether they stand in the relation the line implies, because every clause can pass on its own while the sentence as a whole is still false.** Nothing in a drafting session contradicts a line like that either, which is the same reason decision 59 gives for the under-describing case. |
| 70 | Chapter 14 sources, scope and terminology | Chapter 14 reads arXiv:2508.11053v1 (7 pages), arXiv:2601.10587**v3** (15 pages) and arXiv:2509.00846v1 (8 pages) in full from the PDFs, plus arXiv:1911.02508v2 from outside the corpus (decision 71). All four records re-verified raw against the arXiv abstract pages and the arXiv API. **Paper 28 is the second corpus paper with more than one revision**, after paper 26, so decision 15's clause does work a second time: v1 2026-01-15, v2 2026-04-08, v3 2026-04-10, and the file's own stamp reads v3. Two seeded titles were wrong in capitalisation (papers 27 and 28) and are corrected; paper 29's entry needed no correction at all, the fourth time that has happened. arXiv transliterates paper 28's third author as `Roehrbein` while the PDF byline prints `Röhrbein` beside an ORCID; **the byline wins, and the research note records the difference so a later session does not "fix" it back**. Papers 28 and 29 name venues in their `Comments` fields (the 10th bwHPC Symposium and IJCNN 2025) and both are cited as the arXiv version on decision 48's precedent; the chapter names the bwHPC venue once, because a high-performance-computing symposium is a fact about how the work was reviewed rather than a judgement of it. Paper 28's arXiv "Related DOI" carries the `10.13140` prefix, which is ResearchGate's registrar for self-archived documents and is not a publisher DOI. The chapter adds eleven terms to appendix B, seventeen names to the keep-in-English block, eleven rows to appendix A and four to appendix C (FGSM, PC, IDA, RMSE). It adds no numbered definition and no theorem, two equations, one TikZ figure and one table float. **This is decision 64's object gap for the sixth time and the first chapter to meet it three times at once**: the three papers measure bias concealment, misclassification rate and distance to a synthetic reference, and none of the three is a faithfulness measurement. |
| 71 | **The book cites a second paper from outside the corpus, and decision 34's trigger widens** | Decision 34 admitted seven `m`-prefixed keys to chapter 08 because no corpus paper defines a faithfulness metric. Chapter 14 meets the same wall in a different form: the scaffolding attack is the object of two of its sections, and the only corpus paper that touches it, paper 27, is a *replication* of it. Reading a construction through a seven-page replication when two sections argue from the construction is what decision 34 exists to prevent, so `mfooling` is added, prefix `m` because a corpus key's number is a reading-ladder position and this paper has none. Appendix D still lists exactly 32 papers and does not list it; the reading ladder is unchanged. arXiv's own record for it has no `journal-ref` and **no `Comments` row at all**, so arXiv says nothing about where it was published, while the PDF's own first page carries the AIES '20 reference block and a publisher DOI. That is decision 53's case exactly, so the book cites the arXiv version at v2 and the venue stays in the `note` field rather than reaching the prose. **The widened trigger: decision 34's test was "no corpus paper defines this". The test this adds is "no corpus paper *is* this" - a corpus paper that only replicates, surveys or cites the construction a chapter argues from is not a substitute for it.** |
| 72 | **A symbol collision settled by shape for the second time, and the two conditions restated** | Decision 61 kept both senses of `$K$` because Kolmogorov complexity is always applied while LIME's `$K$` is always bare, and because one of the two names is standard outside this book. `$\varepsilon$` is the second instance and meets both conditions: chapter 13 binds `$\varepsilon_f(b)$`, always subscripted and applied, while paper 28's attack intensity is always a bare `$\varepsilon$`, and a bare `$\varepsilon$` for the magnitude of an adversarial perturbation is standard across that whole literature, including in the FGSM equation paper 28 itself quotes. Renaming it would send a reader to any paper on adversarial examples looking for a letter that is not there. **Four other symbols in the same chapter did not meet the conditions and were renamed the ordinary way** (decisions 29, 41 and 58): the innocuous classifier `$\psi$` becomes `$u$` because `$\psi_i(x)$` is chapter 07's data attribution; the SHAP-magnitude threshold `$v$` becomes `$\tau$` because `$v(S)$` is chapter 04's coalition value; the pixel-edge margin `$h$` becomes `$\eta$` because `$h_x$` is chapter 04's; and the causal weight `$\gamma_i$` becomes `$\lambda_i$` because `$\gamma_k(x)$` is chapter 07's component attribution. The causal graph's vertex and edge sets get no symbol at all, which is decision 58's case, because `$E$` is chapter 07's object set and writing "đỉnh" and "cạnh" out is shorter than resolving the collision. The ratio is the lesson: one collision in five met the shape test, and the test stays hard on purpose. |
| 73 | **Where a source's label does not name what its own column computes, the book reproduces the column and keeps the ranking** | Decision 40 covers a result whose stated hypotheses are too weak, decision 62 a result whose derivation does not follow, and decision 62c the correction that a broken derivation licenses rejecting the derivation and not the conclusion. Paper 29 supplies a fourth case, at the level of a reported statistic rather than an argument. Its table II prints a column headed `RMSE`; reconstructing the column from the table's own cells shows it is the **sum of squared deviations** from the ground-truth column over the two ground-truthed features, with no division by their count and no square root. Eleven of twelve cells reproduce, each within the rounding that four-decimal inputs allow once squared, and two of them exactly at four places, `1.6357` and `3.9193`. **One cell does not**, Shapley Flow on the lung block, and the chapter names it and stops there. The table's own second-best markings are correct under the reconstruction, so the chapter withholds nothing. **The two readings are monotone in one another, so every ranking in the table is identical either way and the paper's conclusion stands.** That is decision 62c applied on purpose rather than learned again: the chapter prints the defect, keeps the result, and says which is which. The general rule: a mislabelled statistic is checked by recomputing it from the source's own printed cells, and the recomputation goes in the research note so the next session does not redo it. **The session learned a second rule the hard way and it is the more useful one.** The first reconstruction reported *two* irreproducible cells, because the note had transcribed the ASV/Stress cell as `0.2600`, the value of the neighbouring column, where the page prints `1.9125`. On that error the chapter said the lung block reproduced four of six and declined a second-best marking that is in fact correct. The cold audit caught it by recomputing from the page rather than from the note. So: **a reconstruction that fails on some cells is evidence about the transcription before it is evidence about the source**, and an irreproducible cell is the first place to re-read, not the first place to report. |

| 74 | **Chapter 15's TOC line fails by naming less, which is decision 59's mode a second time, and its middle clause is a claim no paper makes** | The open item decision 56 opened named chapter 15's line as the one that most reads like a claim inferred from a title, so the three-way check ran before drafting. **More than the papers contain: yes, in the middle clause, and it is the book's own claim rather than an error.** No paper of the chapter says leakage is the bottleneck analogue of unfaithfulness, and `faithful` occurs zero times in the body of all three. The claim is defensible and section 15.2 makes it, marked as the book's, with three limits stated on the page: what is displayed is a layer of the model rather than an explanation an outside method built, so no intervention set has to be published; the excess is defined against an annotated concept set, so changing the set changes the baseline; and no source calls it faithfulness. **Less than the papers contain: yes, and this is the failure that matters.** The line names leakage and says nothing about the measurement of it, which is paper 30's actual contribution and the reason the chapter earns its place in Part V. **The relation between the three: holds, loosely**, and what the phrasing hides is that paper 32 measures no leakage at all, so the arc is two papers on one paradigm and one on another rather than three stages of one story. The line is rewritten in the TOC and in the chapter folder's scope comment. The item decision 56 opened is now two chapters wide, 16 and 17. |
| 75 | Chapter 15 sources, scope and terminology | Chapter 15 reads arXiv:2504.14094**v3** (pages 1-25 in full, 26-39 scanned), arXiv:2506.04237v1 (pages 1-6 in full, 7-10 bibliography scanned) and arXiv:2512.10805**v2** (pages 1-10 in full, 11-15 scanned), plus arXiv:2106.13314v1 from outside the corpus (decision 76). Papers 30 and 32 are the third and fourth corpus papers with more than one revision, so decision 15's clause does work twice more. **The records were re-verified once rather than twice, and that deviation is recorded rather than hidden**: `export.arxiv.org/api/query` returned `HTTP 429 Rate exceeded` to every request that day, over both schemes and with and without a custom User-Agent, so the abstract pages are the only source and the research note says so and says how to redo it. Paper 30 has no journal-ref and a Comments field naming no venue; paper 31 has **no Comments row at all**, which is paper 27's case; paper 32's Comments reads `CVPR 2026`, which is decision 48's case. All three are cited as the arXiv version and paper 32's venue lives in its refs.bib note. **All three seeded entries needed no correction**, the fifth, sixth and seventh time that has happened, including the capitalisation split between paper 30's `Concept-Based` and paper 31's `Concept-based`. The chapter adds seven terms to appendix B and nine keep-in-English rows; CTL, ICL, OIS, CEM and CB-SAE go to appendix C, while NIS goes to the keep block because the prose names it once, which is the CQS and ASV rule cutting a third time. **`CBM` was seeded in appendix C at init and sat unused for fourteen chapters; this chapter earns it** on chapter 13's AIT precedent, introducing it at first use in section 15.1 and using it throughout. Two terminology calls the prose carries: the intervention score is named `điểm số can thiệp` and not `điểm can thiệp`, because the shorter form is a substring of the ordinary phrase `thời điểm can thiệp` that chapter 08 already uses; and `rò rỉ` collides with chapter 11's `rò rỉ tương tác`, named once in section 15.2 on decision 55's rule, with the bare word reserved for this chapter's sense. |
| 76 | **The book cites a third paper from outside the corpus, and decision 34's trigger widens a third time** | Decision 34 admitted seven keys because no corpus paper defines a faithfulness metric; decision 71 admitted `mfooling` because the only corpus paper touching the scaffolding attack merely replicates it. Chapter 15 meets a third form: **paper 31 makes a directional claim about Mahinpei et al., arXiv:2106.13314, that the chapter has to adjudicate**, and it cannot be adjudicated from paper 31 or from paper 30, both of which cite that source for other things. Key `mclm`, prefix `m` because a corpus key's number is a reading-ladder position and this paper has none. Appendix D still lists exactly 32 papers and does not list it. General rule: the book reads a paper outside the corpus when a corpus paper makes a claim *about* it that the chapter has to judge. |
| 77 | **Where a source misreports a third source, the book reads the third; and where two sources merely measure different quantities, it says so instead of calling one wrong** | The decision 40 / 62 / 62c / 73 family covers a source's own hypotheses, derivation, conclusion and reported statistic. This is one level out: a source's report of *another* source. Paper 31 section 4.1 says Mahinpei et al. found hard concept representations leak significantly more than soft ones. **That is backwards**: the source's appendix C states that soft representations \enquote{always perform better than the hard representations when used in the downstream task}, and its section 3 is titled for leakage *in soft representations*. That is the finding, and it is flat. **The second half of this row records a charge the session made and the cold audit made it withdraw**, because the lesson is worth more than the finding. The draft also accused paper 30 of citing Mahinpei et al. in support of \enquote{by definition no leakage can be present in hard CBMs}, a claim whose source concludes the opposite. Rechecking the rendered pages shows paper 30 cites that source for the *existence* of leakage (p. 5) and cites Koh, Kazhdan and Margeloiu, not Mahinpei, for the hard-CBM claim (p. 5 and p. 12). The charge was wrong and is gone from the chapter. **The mistake is decision 62c's exactly**: a session that has just found one error is at its least suspicious of the next thing shaped like it, and one real inversion made a second, symmetrical one feel likely enough to write down without rechecking a citation list. What survives is better than what was withdrawn. Paper 30's \enquote{by definition} is honest inside its own framework, since its excess is measured against a fixed annotated set; the source's warning that leakage \enquote{may always be an issue ... regardless of the form of concept representation} is about a different quantity, what a whole set of concepts carries about the data distribution. Section 15.4 says the two do not contradict each other and names both quantities. **The rule: read the third source; where a report of it is simply wrong, say so; and where two sources look opposed, check first whether they are measuring the same thing, because the interesting answer is usually that they are not.** |
| 78 | **A symbol collision settled by shape for the third time, and two symbols the chapter refuses to name at all** | `$I(z,w)$` for mutual information meets both of decision 72's conditions against chapter 08's bare `$I$` for the infidelity perturbation: mutual information always takes two arguments and that perturbation always stands alone, and `$I$` is standard across the whole of information theory. Three ordinary renames sit beside it (decisions 29, 41, 58): the concept-supervision weight `$\lambda$` becomes `$\beta$` because `$\lambda_i$` is chapter 14's causal discount, the concept count `$k$` is written `$\lvert\mathcal{C}\rvert$` because `$k$` is chapter 08's top-scoring count, and `$\hat c_k$` is an extension row under chapter 07's `$c_k$` rather than a new name, because a CBM concept *is* a component in chapter 07's sense with the extra constraint that it must match an annotated name. **Two symbols get no name at all, which is decision 58's move used twice in one chapter.** Paper 30 writes the concept encoder `$g$` and the final head `$f$`, the two letters appendix A has bound to the readable surrogate and to the model being explained; the book calls them `bộ mã hóa khái niệm` and `đầu phân loại` in words and gives neither a symbol, so `$f$` stays the model book-wide. And the intervention score `$S_{int}$` gets no symbol because `$S$` is chapter 04's coalition, so the Vietnamese name carries it. |
| 79 | **The first validated measuring instrument in the book, and the fourth route to ground truth** | Part IV's charge across seven chapters is that the field's faithfulness metrics have never been checked against an independent criterion, because nobody has one. Paper 30 has one and uses it: table 15.1 correlates CTL and ICL against the intervention score across eight datasets, and CTL beats the prior score OIS on all eight with a `p`-value below 0.05 on all eight. **What makes it possible is that the ground truth arrives with the dataset**, since the concepts are annotated, which is a fourth route beside the three section 14.7 counted: task design (chapter 09), data design, and model design (chapter 14). It is the cheapest of the four because nobody has to construct anything. Section 15.4 states three limits in the same breath, and they are why this does not settle Part IV: the criterion is coarse by its own paper's account, specific without being sensitive; the mutual-information estimator cannot compare representations of different dimension, so the framework loses its baseline on vector-valued architectures and what remains is a raw quantity that cannot be compared against zero; and what is validated is a leakage score rather than a faithfulness metric. **Chapter 16 receives the route and its cost, and chapter 18 receives the pattern**: a validated instrument exists where the ground truth is annotated rather than constructed. Paper 30's own second robustness argument, that the scores vanish on hard CBMs, is recorded here as a consistency check against its own definition and not as a validation, which is the reading decision 77's investigation produced. |
| 80 | **A table-of-contents line can also fail in its organising predicate, and one of its own papers can be the thing that refutes it** | Decisions 24, 28, 47 and 56 are lines promising more than their papers contain; decision 59 and decision 74 are lines promising less; decision 69 is a line whose clauses are each true and whose arrangement is false. Chapter 16's line adds a fourth mode and it is the sharpest of the four. The line read \enquote{the three escape routes the critique points at, **each with its cost**}, and the predicate is what fails: **paper 24 argues at length that the cost of its route is not the obstacle.** `cost`, `expensive`, `power`, `IRB`, `consent`, `budget` and `burden` occur zero times in its seven pages; its one difficulty sentence is withdrawn by the next one (\enquote{Regardless, it is difficult to trust generalizable claims...}); and page 6 names the cost objection and rejects it, \enquote{Undoubtedly there are challenges in moving from testing AI solely with static datasets into testing with humans, but they are not insurmountable}, citing Doshi-Velez and Kim for the proposition that the methods already exist. A chapter written to the line would have reported its own source backwards. **What makes this mode different from the first three: a noun that over-promises is caught by reading the abstract, and a false arrangement is caught by asking how the papers relate, but a predicate is a claim the line makes about *all* its papers at once, and it is only caught by asking each paper whether it agrees.** The general rule this adds: **the re-check has a fourth leg, and it is to read the line's verb, not only its nouns.** The corrected line and what the chapter delivers instead are in the table of contents above; the chapter states the finding on the page in section 16.7 rather than letting the chapter title carry it. |
| 81 | Chapter 16 sources, scope and terminology | Chapter 16 reads arXiv:2605.25052v1 (31 pages), arXiv:2503.16507v1 (7 pages), arXiv:2501.18887**v3** (25 pages) and arXiv:2512.10805**v2** (15 pages), all four re-verified against the PDFs and the arXiv abstract pages on 2026-09-06; the arXiv API was not used, which is chapter 15's deviation a second time and recorded in the note. **Paper 18 gets its record written down for the first time**, because chapter 07 read it before decision 35 started `research/`: its abstract page carries **no `Comments` row and no `journal-ref`**, so arXiv records no venue and the chapter names none, which is paper 27's and paper 31's case rather than decision 48's. Seven sections plus summary and questions; one TikZ figure; one table float; no numbered definition and no theorem. **The chapter adds no term to appendix B and no row to appendix C, which is the first time in the book**, and that follows from reading no new paper: every term it needs is already owned by an earlier chapter and borrowed once here. It adds no row to appendix A either. **This is decision 64's object gap for the seventh time**, and the seventh is the widest: `faithful` occurs zero times in papers 18, 24 and 32, and paper 21 uses it about chuỗi suy luận, so no paper of the chapter uses the word about the thing the chapter is about. Paper 18 is the second tier-A paper verified at zero after paper 20 (decision 34). Two terminology calls the prose carries: `SAE` rather than the written-out name, on chapter 15's settlement, and the word `lối thoát` is marked in section 16.1 as the book's rather than any paper's. |
| 82 | **A chapter that reads no new paper is a different kind of session, and re-reading for a question its own chapters did not ask is what makes it worth doing** | Chapter 16 is the first chapter in the book to add no corpus paper and no key of any kind. The risk was that it would be a recap. What it actually produced: the decision 80 finding; **five corrections to three earlier chapters' research notes**, two mis-transcribed verbatim quotes and a page anchor in chapter 12's, a wrong word and a wrong page anchor in chapter 09's, and a page anchor in chapter 15's, none of which had reached the manuscript; the pinning of chapter 15's open estimate of the CB-neuron count at about 2,169 from table 3's own denominators; and three quantities no earlier chapter had printed, paper 21's 486 of 1,946, paper 32's `r = 0.06` across 1,329 shared concepts, and its 11.88 to 19.82 percent range where the abstract prints the best of three. **The mechanism is worth stating because chapter 18 is the other chapter of this kind:** a chapter that reads a paper reads it for its own question, and the parts of the paper that answer a different question get skimmed. So the sentence in paper 21 that decides chapter 16's whole shape, that route 1 exists because current interpretability tools are judged unusable, sits on page 4 of a paper two chapters had already read in full and neither had a use for. **The rule: a re-reading session picks its question first and re-opens the PDF, and it does not trust the note, because a note records the answers to the question the note's session was asking.** That is decision 73's second half generalised from a transcription to a whole reading. |
...
14. **Giải thích dưới tấn công** - three papers standing in three different relations to the word "attack", which the initialized line collapsed into one story (decision 69). Paper 27 replicates the scaffolding attack of Slack et al. and then proposes an ensemble defence; the attack itself is in no corpus paper and is cited directly (decision 71). The chapter reads that attack's setup as a third route to constructed ground truth, forced on the attacker because he cannot hide what his own model uses without knowing it, and names its cost. Paper 28 reverses the direction: SHAP replaces the gradient in a white-box evasion attack on image classifiers, so SHAP is the weapon and the classifier the victim, and its misclassification rate is read as a cheap necessary condition on the scores that guided it. Paper 29 has no adversary anywhere; it repairs SHAP's correlation-against-causation weakness with causal discovery and scores the repair with the insertion metric of chapter 08. `faithful` occurs zero times in all three. The chapter's own bequest to chapter 18 is one requirement on a future instrument, that a method must publish the intervention set its scores are relative to, which it pairs with the requirement section 13.6 derives from the theoretical ceiling. Papers 27, 28, 29.

### Phần V - Những lối thoát

15. **Mô hình nút thắt khái niệm và rò rỉ** - concept bottleneck models, and the price of the escape route Part V opens with. Concept leakage: the model hides task information inside the concept activations the overseer reads, so the bottleneck stays complete in form while it has already gone hollow. The chapter reads that as the bottleneck analogue of unfaithfulness and says so as **the book's own claim, which no paper of the chapter makes**, with three limits stated on the page (decision 74). Paper 30 supplies what Part IV said the field does not have: an information-theoretic instrument, two scores against a stated baseline, **validated against a criterion its authors did not choose**, which works because the ground truth arrives annotated with the dataset - a fourth route beside the three section 14.7 counted (decision 79). It also gives four causes of leakage and a design guidance that ends in a refusal. Paper 31 is the field's own catalogue: nine weaknesses across two paradigms, of which leakage is one, and no architecture in its table meets more than four of six properties. Paper 32 measures no leakage at all; it separates whether an internal unit has a readable name from whether forcing it changes the output, and finds under a fifth of a sparse autoencoder's neurons have both. `faithful` occurs zero times in all three bodies, a second whole tier of the reading ladder after chapter 14's. Papers 30, 31, 32, plus one key from outside the corpus (decision 76) because paper 31 states that paper's central comparison backwards and the chapter has to say so.
16. **Ground truth dựng sẵn, human study, và mechanistic interpretability** - the three escape routes the critique points at, and what each one actually charges. **The initialized line said "each with its cost", and one of the chapter's own papers argues against that predicate (decision 80)**, which is a fourth kind of TOC failure: not a noun that over-promises, under-describes or mis-arranges, but an organising predicate a source contradicts. What the chapter delivers instead: the four ways of constructing ground truth the book has now met, and the one thing every one of them forces; then paper 21's own accounting of what its construction reaches, which is 486 of 1,946 step-level labels and one step type of four, with the representativeness charge marked as the book's because the paper answers it; then paper 24, whose route costs little and validates a different relation, since it prints `cost`, `expensive` and `power` zero times and rejects the cost objection outright while its own ceiling is "explainability claims" and "the needs of end-users"; then paper 18, a position paper that *files* mechanistic interpretability under attribution rather than practising it, and that states the evaluation gap as **shared across all three attribution arms**, naming no ground truth for components anywhere in 25 pages; then paper 32 measuring what a readable name is worth, where the share of units that are both readable and steerable runs 11.88 to 19.82 percent across three models, moves to 29.14 percent on one change of threshold, and whose steerability correlates at `r = 0.06` across two downstream models on 1,329 shared concepts. **The chapter's finding, and it is the book's own: the three are not parallel exits.** Paper 21 takes route 1 because it judges route 3 unavailable; paper 18 says route 3 inherits the missing instrument rather than escaping it; and what each route charges is a change of question rather than a price. `faithful` occurs zero times in papers 18, 24 and 32. Papers 21, 24, 18, 32 re-used; no new corpus paper, and the first chapter in the book that reads none.
...
| 13 | drafted | Drafted and audited 2026-09-04 from arXiv:2504.20676v2, pages 1--55 body and 56--61 appendix read in full from the PDF; pages 61--65 are the bibliography and were scanned only. Orientation note dead, so drafted from the PDF alone. **The first corpus paper with more than one revision**, which is the first time decision 15's "cited as the revision read" clause has done any work: v1 is 2025-04-29, the book cites v2 of 2025-11-03. The revision and the record were re-verified raw against the arXiv abstract page and the arXiv API; no journal-ref, no Comments field, no venue-tagged dblp entry, so the chapter cites the arXiv version and names no venue. Seven sections plus summary and questions; one TikZ figure; three numbered definitions and three numbered theorems; the book's third table float; **the book's first `proof` environment** (decision 63). It formalizes an explanation as a function approximating the model inside a bit budget, proves the complexity gap theorem in four lines, measures how high the ceiling stands for random Boolean functions, Lipschitz functions and five interpretable model classes, reads the intrinsic-dimension escape, then spends section 13.5 on the paper's single sentence about LIME and SHAP and section 13.6 on what the ceiling does and does not settle. **The chapter prints no measurement, because the paper reports none**: every two-part number in it is a numbered result or a section of the source, all recorded in `research/ch13-gioi-han-ly-thuyet.md`. Full LuaLaTeX build clean (162 pages, no overfull boxes, no undefined references, `build/main.ilg` 0 warnings); mechanical prose gate clean; the rendered chapter pages were inspected, which is how figure 13.1's placement between theorem 13.4 and its proof was caught and fixed. Runs 13 printed pages, 105 to 117. Its TOC line was the first init-written line since decision 24 to survive its paper (decision 59), and it declines to build on one of the paper's own corollaries (decision 62). Status stays drafted until the author's review. Reading-flow pass 2026-09-05: theorem 13.4's proof is shown not to prove it and is replaced by a packing argument over bump functions (decision 66), which reverses what 13.4 and 13.6 concluded about the curse of dimensionality and supersedes part of decision 62b (decision 62c); section 13.2 gains the two-point example separating expected from worst-case error; the word-count forensics in 13.1 compress to the load-bearing absences; the tomtat's longest bullet splits and the questions gain a numeric check of the packing count. Chapter 16's session fixed one defect here that no gate could see: review question 6 used `đầu vào ngoài phân phối`, chapter 08's term, exactly once, wrapped across two lines inside the indented `cauhoids` block, and the chapter glossed it nowhere, so the Gloss check never reported the omission. The term is now on one line with its `\tn`; the open item on wrapped terms carries the mechanism. |
| 14 | drafted | Drafted and audited 2026-09-05 from arXiv:2508.11053v1, arXiv:2601.10587**v3** and arXiv:2509.00846v1, all three read in full from the PDFs, plus arXiv:1911.02508v2 fetched from arXiv because it is not in the corpus (decision 71). All four records re-verified raw against the abstract pages and the arXiv API. **The first chapter in this book to read three corpus papers**, and the first to find that its three papers do not stand in the relation its TOC line asserted (decision 69). Seven sections plus summary and questions; one TikZ figure; one table float; two equations; no numbered definition or theorem, which is a first for Part IV and follows from the papers rather than from a choice. Runs 18 printed pages, 129 to 146, the longest chapter in the book. It reads the scaffolding attack as the third route to constructed ground truth in the book, forced on the attacker rather than chosen by an evaluator, and names its cost; it reads paper 28's misclassification rate as a cheap necessary condition on the scores that guided the attack, holding it to `mức quan trọng` rather than to `độ trung thực`; and it reads paper 29's repair as being scored by the very instrument Part IV has spent six chapters calling unvalidated. **Verified absence: `faithful` occurs zero times in all three papers**, which is the strongest in the book because it is a whole tier of the reading ladder rather than one paper. **Six defects in the sources are named and declined**: paper 27 gives one threshold three values and reports its own method degrading inside that spread, promises plural configurations against one in the body, and justifies its construction with a false claim about SHAP's range that its own background section contradicts; paper 28's equation 5 implements one of the two movements its prose describes; paper 29's missingness theorem proves a different statement from the axiom it names, and its one-line extension of consistency to the normalised values is false because the normalising factor depends on the model. **One finding is the book's own arithmetic** (decision 73): paper 29's `RMSE` column is a sum of squared deviations, reproduced on eleven of twelve cells, with the ranking unaffected. That reconstruction was wrong in its first form and the cold audit corrected it, which decision 73's second half records. Full LuaLaTeX build clean (191 pages, no overfull boxes, no undefined references, `build/main.ilg` 0 warnings); mechanical prose gate clean; the rendered chapter pages were inspected, which is how figure 14.1's floating above its own introduction and table 14.1's caption promising a column the tabular did not have were both caught and fixed. Closes the Part IV limitation-statement log. Status stays drafted until the author's review. |
| 15 | drafted | **Opens Part V.** Drafted and audited 2026-09-06 from arXiv:2504.14094**v3**, arXiv:2506.04237v1 and arXiv:2512.10805**v2**, all read from the PDFs, plus arXiv:2106.13314v1 fetched from arXiv because it is not in the corpus (decision 76). Both CBM orientation notes were dead, as the open item predicted, so the chapter drafted from the PDFs alone and closed that item. Seven sections plus summary and questions; one TikZ figure; one table float; one numbered definition; no theorem. Runs 16 printed pages, 147 to 162. The second chapter to read three corpus papers, and the first where **one of its papers states a fourth paper's finding backwards**, which is why it cites a third outside-corpus key (decisions 76 and 77). **Its TOC line failed by naming less**, decision 59's mode a second time: the line named leakage and missed the measurement of it (decision 74). The chapter's centre is that measurement. Paper 30 validates two leakage scores against the intervention score across eight datasets, which is the first validated instrument the book has met, and section 15.4 spends as much room on the three things the validation does not reach as on the validation itself (decision 79). It also records that the paper's second robustness argument, vanishing scores on hard CBMs, is a consistency check against its own definition rather than a validation. **Three defects named and declined, and a fourth charge withdrawn**: paper 30's claim that both scores \enquote{systematically outperform OIS across all datasets} is true of CTL on all eight and false of ICL on three, by its own table 2; paper 31's sentence about hard against soft representations reverses its cited source; and **paper 32's `+14.0%` steerability gain is the sum of its two per-metric gains where the other five figures in the same sentence are their mean**, so the value under the paper's own rule is +7.0% and the abstract's +14.5% headline becomes 12.2%, which is the book's own arithmetic on a printed table for the second time after decision 73. Every ranking in both papers is unchanged by both reconstructions. **The withdrawn charge is decision 77's second half**: the draft also accused paper 30 of citing that same fourth paper for a claim its conclusion contradicts, the cold audit flagged the attribution, and rechecking the pages showed paper 30 cites three other works for it. The cold audit found four further errors of fact, all fixed: the intervention score defined with its subtraction reversed, alignment leakage reported on four datasets where the paper names five, `AUC` used for the quantity appendix C calls AUROC, and chapter 09's two named readings borrowed for a distinction they do not name. **Extends the running example into Part V**: the function chapters 01 to 08 use, thresholded at 1, is the OR of its two features, which is the illustrative task paper 30 uses, so section 15.2 computes the concepts-task baseline and the leakage ceiling on it by hand. `research/ch15-nut-that-khai-niem.md` anchors every number, both reconstructions, and the one deviation in how the records were verified. Full LuaLaTeX build clean (208 pages, no overfull boxes, no undefined references, `build/main.ilg` 0 warnings); mechanical prose gate clean; the rendered chapter pages were inspected. Status stays drafted until the author's review. **Chapter 16's session fixed one defect here that shipped past every gate**: section 15.7's opening paragraph carried a literal carriage return where a backslash belonged, so the source read `định nghĩa~<CR>ef{def:ch15-hai-diem-so}` and the page printed the macro name as words. It is not an undefined reference, because there was no reference left to resolve, and the rendered-page inspection this row records did not catch it. A book-wide byte scan for stray control characters and for macro-shaped tokens missing a backslash found it as the only occurrence. The same paragraph also repeated `và` across a line break. |
| 16 | drafted | **The first chapter in the book that reads no new paper** (decision 82). Drafted and audited 2026-09-06 from arXiv:2605.25052v1, arXiv:2503.16507v1, arXiv:2501.18887**v3** and arXiv:2512.10805**v2**, all four re-read from the PDFs for one question none of their own chapters asked; records re-verified against the abstract pages, the arXiv API being unreachable again, which is chapter 15's deviation a second time. Seven sections plus summary and questions; one TikZ figure; one table float; no numbered definition and no theorem; no new bib key, no appendix A row, no appendix B block and no appendix C row, all four of which are firsts. Runs 13 printed pages, 163 to 175. **Its TOC line failed in a fourth way** (decision 80): the predicate, not a noun, and paper 24 is the thing that refutes it. It collects the four ways of constructing ground truth into figure 16.1 and prices route 1 on paper 21's own table 10, 486 of 1946 step labels and one of the four step types that table counts, with the representativeness charge marked as the book's because the paper answers it on p. 9. It reads route 2 as affordable and aimed elsewhere, and route 3 as inheriting the missing instrument, on paper 18's statement that the evaluation gap runs across all three attribution arms. Table 16.1 extends chapter 15's four quadrants to three models, 11.88 to 19.82 percent. **Extends the running example**: the function chapters 01 to 08 use, thresholded at 1 and at 4, gives the OR and the AND tasks, and section 16.3 works out that exactly one of the two output values determines the dependence set under a single-flip intervention set, which is what paper 21's phrase \enquote{partially recoverable} means made computable. `research/ch16-ba-loi-thoat.md` anchors every number and records the five corrections this session made to chapters 09, 12 and 15's notes. **The 2026-09-06 cold audit was productive and three of its findings were factual.** It caught that `ground truth` occurs three times in paper 18 and not two, the third being invisible to a line-based search because it wraps, which is this session's own subject arriving through a research tool rather than the gate; that 18.84 percent is not the high end of the 11.88 to 19.82 range and the figure sits in paper 32's introduction rather than its abstract; and that the r = 0.06 passage had dropped the paper's own explanation for the low correlation and had carried a statistic about 1329 CB concepts across to the 65,536 SAE neurons of the quadrant table. It also caught that paper 18's task-specific evaluation is downstream utility and not the chapter's first route, so section 16.5 now maps two of paper 18's three approaches rather than three, and review question 8 asks for the third; that paper 24's two named objections are bias and representativeness rather than cost, which sharpens decision 80 rather than weakening it; that appendix F.3's title is not what question 8 called it; and a self-contradicting sentence in 16.2, a doubled announcement in 16.6, three announcing sentences, a two-word punchline close, a rhetorical question, the doubled \enquote{Thứ thứ} construction, and a figure label pointing at section 14.2 where the prose points at 14.3. **Rejected on the record: one.** The auditor could not find paper 18's \enquote{operates primarily at test time} and read the chapter's clause as an unmarked inference; it is on p. 5 in section 3.3, verified from the extraction with the page count, so the sentence and its citation stand. All other findings are fixed. Re-checking the audit's `ground truth` catch also forced a re-count of **every** phrase absence the chapter prints, under whitespace normalisation; all of them held except paper 24's `cost`, which occurs once inside the surname *Lacoste* in the bibliography, and the chapter now says so. Full LuaLaTeX build clean (221 pages, no overfull boxes, no undefined references, `build/main.ilg` 0 warnings); mechanical prose gate clean; the rendered chapter pages were inspected, which is how a body-text `itemize` that would have been the only one outside a summary box in sixteen chapters was caught and turned into prose, along with `sparse autoencoder` written out where chapter 15 had settled `SAE` and an ambiguous \enquote{hơn bốn phần mười} that is now a stated factor. Status stays drafted until the author's review. |
...
| App A | drafted | Seeded by chapter 03, extended by chapters 04 through 07, and made a longtable by decision 30. The chapter 01-06 audit added rows for the feature count and for Grad-CAM's `$Z$`, renamed the player set to `$\mathcal{P}$` (decision 41), and put `hàm nhân` back in Vietnamese. The 2026-09-02 opaque-prose pass added rows for `$E$`, `$\theta^{*}$` and `$\mathcal{L}$`. Chapter 12 added `$r$` and `$R^2$`, the first two rows that belong to a statistical procedure rather than to an attribution method, so their four right-hand columns are empty like chapters 06 to 08's rows. Chapter 11 added one row, `$\varphi_{j\to i}(x,f)$`, placed directly under `$\varphi_j(x,f)$` because it extends that symbol rather than competing with it, and carrying the note that paper 23 writes its two arguments the other way round. It deliberately added no row for the serial attribution (decision 58). Chapter 13 added thirteen rows, the largest single addition this table has had, and four of them rename a symbol the source uses (decision 61): `$b$` for the bit budget the paper writes `$k$`, `$\mathcal{G}_b$` for the class it writes `$\mathcal{I}_k$`, `$\mathcal{D}$` for the distribution it writes `$D$`, and `$\rho$` for the output distance it writes `$d$`. It also added `$d$` itself, which chapter 06 had bound and never given a row, and extended chapter 12's `$r$` row to carry the neighbourhood radius. `$K(f)$` is the row to read before adding any symbol here, because it is the one collision the book settled by keeping both names. Chapter 14 added eleven rows and set a ratio worth keeping: five of its symbols collided, four were renamed the ordinary way (`$u$` for `$\psi$`, `$\tau$` for `$v$`, `$\eta$` for `$h$`, `$\lambda_i$` for `$\gamma_i$`), one met `$K(f)$`'s shape test and kept both names (`$\varepsilon$`, decision 72), and the causal graph's vertex and edge sets got no symbol at all because `$E$` was taken. `$v_c(S)$` is an extension row rather than a new name, placed under `$v(S)$` the way `$\mathcal{G}_b$` sits under `$\mathcal{G}$`. Chapter 15 added five rows and used all three ways of gapping a collision in one chapter (decision 78): two ordinary renames (`$\beta$` for `$\lambda$`, and `$\lvert\mathcal{C}\rvert$` written out rather than taking `$k$`), one extension row (`$\hat c_k$` under `$c_k$`, whose text was rewritten to say how chapter 15 narrows it), and one kept-by-shape (`$I(z,w)$`, the third after `$K(f)$` and `$\varepsilon$`). It also refused two symbols outright: paper 30's `$g$` and `$f$` for the concept encoder and the head, which collide with the surrogate and the model, and `$S_{int}$` for the intervention score, which collides with the coalition. Read the `$I(z,w)$` row before adding any symbol that a whole field already writes one way. **Chapter 16 added no row, the first chapter to add none**, and that follows from decision 82: it reads no new paper, so it meets no new symbol. The one quantity it prints that could have taken a symbol, paper 32's Pearson correlation across two downstream models, is written `$r$` and is chapter 12's existing row used a third time. |
| App B | drafted | Nine per-chapter blocks plus the keep-in-English block, 69 terms under the Gloss check. The chapter 01-06 audit added ten terms the prose used but the appendix never listed, dropped an unused `tích phân đường` row, and moved `baseline` to the keep-in-English block under decision 38. Chapter 09 added its own block of thirteen, `mức quan trọng` to chapter 08's block, and ten metric and dataset names to the keep-in-English block, SCM among them with no expansion (decision 46). Chapter 10 added a block of four, six names to the keep-in-English block, and gave `tính cục bộ` to chapter 03 and `tính ổn định` to chapter 08 rather than claiming them (decision 50); it deliberately added no row for the survey's `fidelity` (decision 49). Chapter 12 added a block of three (cảm nhận của người dùng, thang Likert, lực kiểm định), gave `lời giải thích phản thực` to chapter 07 on decision 27's precedent a fourth time, extended the `sparsity` keep-row to carry its second collision (decision 55), and added eight keep-in-English rows: six of paper 25's metric names, `M4`, and `Combined Quality Score`. Chapter 11 added a block of five (tương tác bậc hai, attribution nối tầng, rò rỉ tương tác, hiệu ứng riêng, tính hiệu quả phân tầng) and nineteen keep-in-English rows: the paper's five coined names, three method names it attributes to others, STII and SOP pointing at appendix C, Recall@K, AttnLRP, and the four model families and four datasets the three experiments run on. **Counted from the gate's own policy line 2026-09-02, after chapter 11: twelve per-chapter blocks, 91 terms under the Gloss check.** The "75 terms" this row once claimed and the "86" that replaced it both went stale within a session, because a pass that adds terms without re-counting leaves the number behind; the `gloss=` figure on check-chapter.ps1's printed policy line is the authority, not this row, so re-read it rather than trusting the number here. Chapter 13 added a block of twelve and two keep-in-English rows (`Lipschitz`, which chapters 05 and 06 had been using bare since before this appendix listed it, and `EU AI Act`), and gave `lời nguyền số chiều` to chapter 07 on decision 27's precedent a fifth time. It also **removed a term it had just added**, `sai số xấp xỉ`, because the gate showed chapter 05 using the same phrase for the error of a Riemann sum: two ordinary technical phrases for two objects, which decision 55's test sends neither way, so the row goes rather than the collision being named (decision 60). **Counted from the gate's own policy line 2026-09-04, after chapter 13: thirteen per-chapter blocks, 104 terms under the Gloss check.** Chapter 14 added a block of seven and sixteen keep-in-English rows: the three tabular datasets of the scaffolding experiment, the four image datasets and the architecture of paper 28, and the five SHAP variants paper 29 benchmarks against, plus `BASIC SHLIME`, `Brier` and a pointer row for `FGSM`. **Four rows it added were removed again in the same session**, on the rule that took out chapter 05's unused `tích phân đường`: `bộ phân loại đối kháng`, `hàm giá trị nhân quả`, `hiệu ứng trực tiếp` and `hiệu ứng gián tiếp` were each in the block for one gate run and none of the four strings occurs anywhere in the prose, because the chapter says `mô hình đối kháng` in ordinary Vietnamese and never needs the other three as terms. It deliberately added **no** keep-row for `IBS`, because the prose names the two biomedical datasets only as "hai bộ dữ liệu y sinh thật" and a row for a name the prose never says is clutter; and its `ASV` row records that the abbreviation is expanded in its source but used once in the prose, so appendix C has no row for it, which is the CQS rule cutting the same way twice. **Counted from the gate's own policy line 2026-09-05, after chapter 14: fourteen per-chapter blocks, 115 terms under the Gloss check.** Chapter 15 added a block of seven (mô hình nút thắt khái niệm, rò rỉ, rò rỉ khái niệm-nhiệm vụ, rò rỉ liên khái niệm, rò rỉ căn chỉnh, điểm số can thiệp, khả năng điều khiển) and nine keep-in-English rows: NIS, the six dataset names its results come from, ImageNet-1K and CLIP. It named its intervention score `điểm số can thiệp` rather than `điểm can thiệp` after the gate showed the shorter form matching inside chapter 08's ordinary `thời điểm can thiệp`, which is a new way for a glossary row to go wrong: **not a collision of meaning but a term that is a substring of a phrase the book already writes.** It also left `bộ mã hóa khái niệm` and `đầu phân loại` out of the block on purpose, on the reasoning the open item below records: both are descriptive Vietnamese compounds whose English a reader reconstructs, and adding them would put a parenthesis in every section for no gain. **Counted from the gate's own policy line 2026-09-06, after chapter 15: fifteen per-chapter blocks, 118 terms under the Gloss check.** **Chapter 16 added no block and no keep-in-English row, the first chapter to add nothing here**, still 118 terms on the gate's policy line 2026-09-06; it borrows twelve terms from eight earlier chapters and glosses each once, which is what a chapter that reads no new paper does. Its session also found that **the Gloss check is systematically blind to a multi-word term wrapped inside `tomtat` or `cauhoids`**, measured at 21 hidden uses of which 13 are in summary files, and fixed the one that hid a real omission in chapter 13; the open item below carries the mechanism and the corrected diagnosis. |
| App C | drafted | **Fifteen abbreviations, counted 2026-09-02**: ten seeded at init, plus Grad-CAM (used eighteen times, never expanded in the prose), RISE and ROAR from chapter 08, and AUROC and FUR from chapter 09. The "thirteen" this row previously claimed had missed RISE and ROAR. SCM is deliberately absent under decision 46. Chapter 12 added none and that is a decision rather than an omission: it introduced CQS for paper 25's Combined Quality Score, then never used the abbreviation again because the prose says "điểm gộp" throughout, so the row was removed and the English name went to appendix B's keep-in-English block instead. An abbreviation the book does not itself use does not belong in a table of the abbreviations the book uses. **Seventeen after chapter 11**, which added STII (Shapley-Taylor interaction index) and SOP (sum of power); both are expanded in paper 23 on p. 3 and both are used more than once in the prose, so both clear decision 46. **Chapter 13 added none and instead earned one that was already there.** AIT was seeded at init and no chapter had used the abbreviation in eleven chapters of prose, which under the rule this row states for CQS would have made it a candidate for removal. Chapter 13 introduces it at first use in section 13.1, expanded, and uses it three more times, so the row now describes an abbreviation the book actually uses. The lesson is that the CQS rule cuts both ways: an unused row is either removed or earned, and a session that touches the subject of a seeded abbreviation should decide which. Still seventeen, counted 2026-09-04. **Twenty-one after chapter 14**, which added FGSM (expanded in paper 28 p. 53), PC and IDA (both expanded in paper 29 p. 1) and RMSE (expanded in paper 29 p. 7); all four are used more than once in the prose, and PC and IDA earned their second use during drafting rather than being abbreviated once and left. **The chapter also declined three candidates on this row's own rule**: OOD, because the prose writes "bộ phát hiện ngoài phân phối" in Vietnamese throughout and never uses the abbreviation; DAG, which appears only inside IDA's own expansion; and ASV, which is expanded in its source but used once, so it sits in appendix B's keep-in-English block instead. Counted 2026-09-05. **Twenty-six after chapter 15**, which added CEM, CTL, ICL, OIS and CB-SAE, all five expanded in their sources and all five used many times in the prose. It declined NIS on this row's own rule, expanded in its source but named once, which sends it to appendix B beside CQS and ASV. **And it earned CBM**, the second time this row's rule has cut that way after chapter 13 earned AIT: the row was seeded at init, no chapter had used the abbreviation in fourteen chapters of prose, and chapter 15 introduces it at first use in section 15.1 and then uses it throughout rather than repeating a five-word Vietnamese phrase. Counted 2026-09-06. **Still twenty-six after chapter 16, which added none and needed none**: it reads no new paper, and the only abbreviation it leans on heavily, SAE, chapter 15 already earned. It writes `SAE` rather than the expanded name throughout, on the one-name-per-concept rule, after its first draft had written the full form in the prose where chapter 15 had settled the abbreviation. Grows with the chapters. |
...
- **Cohesion:** every section opens with a sentence that ties to the previous
  ...
  Chapter 13 adds "lý thuyết thông tin thuật toán", "độ phức tạp Kolmogorov",
  "máy Turing phổ dụng", "lớp khả diễn giải", "hàm sai số giải thích", "hàm độ
  phức tạp giải thích", "không suy biến", "biến phân bị chặn", "số chiều đếm
  hộp", "giả thuyết đa tạp", "miền khả thi" and "bộ ba bất khả"; Lipschitz and
  EU AI Act stay English. Chapter 14 adds "phép dựng giàn" (scaffolding), "bộ
  phát hiện ngoài phân phối", "tấn công né tránh", "hộp trắng", "cường độ tấn
  công", "khám phá nhân quả" and "cường độ nhân quả". It settles three things
  that the audit had to force. **"ground truth" stays English here too**: the
  first draft slipped into "câu trả lời đúng" in five places while writing
  `ground truth` in English elsewhere in the same sections, which is chapter
  08's slip a second time and the reason this rule is written the way it is.
  **The adversary has one name, "bên tấn công"**, against "người tấn công", "kẻ
  tấn công" and "bên triển khai mô hình", which the first draft rotated. And
  **"độ chính xác" is left to the two senses that were already settled**, the
  axiom of chapter 04 and a classifier's accuracy: the first draft added a
  third for the share of explanations still naming the protected attribute, and
  that quantity is now written out rather than named. The protected attribute
  itself is "thuộc tính được bảo vệ" and not "đặc trưng thiên lệch". The dataset
  names, the five SHAP variants of chapter 14's third paper, FGSM and
  EfficientNetB7 stay English on the reasoning chapter 08 gave for its own
  metric names; note that the variant chapter 04 settled as KernelSHAP keeps
  that spelling here even though the paper writes it as two words.
...
- **Research:** the cliff is crossed. `research/` holds a note from every
  chapter since 08 (decision 35), which after chapter 14 means notes for 08
  through 14 plus the reading-flow note, so the Numbers and Verbatim checks are
...
- **Six orientation notes are dead locally** (papers 02, 07, 20, 26, 30,
  ...
  **CLOSED 2026-09-06 by chapter 15**, which drafted papers 30, 31 and 32 from
  the PDFs alone; both CBM orientation notes were still dead. No chapter left in
  the book depends on this item. Nothing was lost by it either: every paper
  whose note went missing has now been read from source, and both sessions that
  did that reading found something in the PDF an orientation note would not have
  carried, the dropped Greek letters below and, in chapter 15's case, table
  cells whose rows and columns interleave under extraction. **The general lesson
  is the one the trap below states and it is now confirmed twice: the note was
  never the cheaper path, only the faster-feeling one.**
...
- **The limitation-statement log for chapter 18.** Each Part IV drafting
  ...
  so its job has to be describing where the two differ.

  **CLOSED 2026-09-05 by chapter 14, which logged the last three papers.** All
  seven Part IV chapters are now in `research/`, and the log's shape is not one
  shape. Five papers, 21 through 25, name the same blocking limitation from five
  directions: measured to fail, recorded as having no standard, named as the
  next step not taken, counted as almost never run, and run once with the
  answer not tracking. Three do not name it, and the three non-crossings differ
  from each other as much as they differ from the five: paper 26 wants a
  computable stand-in for an uncomputable quantity, paper 28 wants
  better-generalising models, paper 29 wants better causal discovery. **Paper 27
  is the one crossing among the three robustness papers and it is the strongest
  entry in the whole log**, because it is a paper proposing an explanation
  method whose own future-work section says the authors never measured whether
  the method explains anything, and separately that the comparison its
  conclusion rests on was not made on a fair scale. That is chapter 11's
  crossing, the missing instrument named from inside a methods paper, arriving a
  second time and this time from inside a paper whose whole purpose is to repair
  explanations. Chapter 18 takes the intersection of these statements and now
  has the complete set; what it also has, and should not lose, is the count of
  papers that intersect nothing, because three of eight is a fact about the
  shape of the gap.
...
- **The remaining init-written TOC lines have not been re-checked against their
  ...
  survive its paper. **Chapter 14's is checked and it failed in a third way**
  (decision 69): each of its three clauses is defensible about some paper, and
  the relation the line asserts between the three is wrong. So the item is three
  chapters wide now: 15, 16 and 17.
  ...
  next to check. **Chapter 13 and chapter 14 each widened what the check is
  for.** Chapter 13's line was not wrong, it was short: it named the ceiling and
  said nothing about the third of the paper that is about AI regulation,
  including one of the paper's three headline results. Chapter 14's clauses were
  each true of something and collectively false, which no clause-by-clause read
  catches. So the re-check is now three-way: does the line promise more than the
  papers contain, does it promise less, and where it names several papers, do
  they stand in the relation it implies. The second and third are the harder
  ones to notice, for the same reason in both cases, that nothing in a drafting
  session contradicts them. This is still cheap to discharge, one abstract per
  chapter before any drafting plus one question about the arrangement, and it
  closes when the last of those three is drafted.

  **Chapter 15's is checked and it failed the second way** (decision 74), so the
  item is two chapters wide now, 16 and 17. Its line named leakage and said
  nothing about the measurement of leakage, which is the whole reason the
  chapter belongs in Part V. That makes the score across seven checked lines one
  survivor, five over-promising and one under-describing twice over, and it
  sharpens the prescription rather than changing it: **the leg of the check that
  keeps finding things is the second one, and the question that finds them is
  not "is this line true" but "what is the strongest thing in these papers, and
  does the line name it".** Chapter 15 also adds a fourth leg worth one sentence
  of thought per line: whether any clause of the line is a claim the *book*
  makes rather than a claim its papers make. That is not a defect when the
  chapter owns it on the page, and it is a defect when the chapter lets the
  chapter title carry it silently.

  **Chapter 16's is checked and it failed in a fourth way** (decision 80), so
  **the item is one chapter wide now, 17 only**. Its line's nouns were fine and
  its arrangement was arguably fine; what failed was its verb. It said the three
  routes come \enquote{each with its cost}, and paper 24 spends a passage
  rejecting the claim that its route's obstacle is cost. That makes the score
  across eight checked lines one survivor, five over-promising, one
  under-describing twice over, one mis-arranging, and one whose predicate a
  paper refutes. **The prescription gains a fourth leg and it is the cheapest of
  the four to run: read the line's verb and ask each paper whether it agrees
  with it.** The three legs already there ask what the line's nouns name; none of
  them looks at the claim the line makes across all its papers at once, which is
  where a one-sentence table-of-contents line does most of its work. Chapter
  17's line reads \enquote{chain-of-thought as self-explanation; the Part IV
  critique transferred to LLMs explaining themselves}, and the verb to check is
  `transferred`.
...
- **Five chapter-owned terms sit in no appendix B block.** The 2026-08-24
  ...
  **Chapter 15 adds two more and declines them for the same reason.** `bộ mã hóa
  khái niệm` and `đầu phân loại` are the two halves of every model in the
  chapter and neither is in appendix B. Both are descriptive Vietnamese
  compounds whose English a reader reconstructs unaided, which is a different
  case from chapter 04's `liên minh` and chapter 06's `độ đo`, and it suggests
  the rule this item is looking for has two clauses rather than one: a central
  technical term goes in the block, and a central *descriptive compound* does
  not, the test being whether a reader who has never met the English could guess
  it. **Seven terms now sit outside the blocks.** Chapter 15 also found a second
  way a glossary row can go wrong, unrelated to this item but worth recording
  beside it: its intervention score was first named `điểm can thiệp`, which is a
  substring of the ordinary phrase `thời điểm can thiệp` that chapter 08 already
  writes, so the Gloss check reported chapter 08 as failing to gloss a term
  chapter 08 does not use. The term was renamed to `điểm số can thiệp`. **A
  glossary row has to be checked against the phrases the book already writes,
  not only against the terms it has already bound.**
- **A multi-word glossary term wrapped across a source line is invisible to the
  Gloss check, which lets a shorter term fire inside a longer one.**
  `check-chapter.ps1` builds each term's pattern with `[regex]::Escape`, so the
  space in `rò rỉ tương tác` matches one literal space; a use of that term
  broken over two source lines arrives in the check's body text with a newline
  and two indent spaces between the words and matches nothing. Chapter 11's
  tomtat had exactly that, and it stayed silent for four chapters because no
  shorter glossary term overlapped it. Chapter 15 bound `rò rỉ`, the check found
  the bare word inside the unmatched long one, and reported chapter 11 as
  borrowing a term from chapter 15. Reflowing chapter 11's line fixed both that
  and a real omission it had been hiding, that the tomtat never glossed its own
  `rò rỉ tương tác`. **The reflow is a workaround, not the fix**: any later
  chapter can reintroduce it by wrapping a two-word term at the wrong column.
  The fix is a one-line change to the script, matching `\s+` between the words
  of a term rather than a literal space, and it is a fix to an existing check
  rather than a new one, so it reaches every book at once. Proposed as a diff in
  chapter 15's pull request; this item closes when that lands or when the author
  declines it on the record.

  **Chapter 16's session traced the mechanism in the script and the diagnosis
  above is wrong in its cause, right in its conclusion, and understated in its
  scope.** `ConvertTo-GlossSections` assembles a section's body as
  `line + ' '` per source line, so the newline never reaches the body text: a
  wrapped term arrives as `word word` when the continuation line is **flush
  left**, and the check sees it normally. What breaks it is the continuation
  line's own **indentation**, which survives into the body and makes the gap two
  or more spaces. So the failure is not "wrapped at the wrong column"; it is
  **"wrapped inside an indented environment"**, and in this book the only
  systematically indented environments are `tomtat` and `cauhoids`. Measured
  across the whole manuscript on 2026-09-06: of 149 multi-word glossary terms,
  **134 wrapped uses are still visible to the check and 21 are not, and 13 of
  the 21 are in `08-tom-tat-va-cau-hoi.tex` files.** The gate is therefore
  systematically blind in exactly the two blocks where a chapter restates its
  own terms, which is a worse exposure than the item assumed, and chapter 11's
  instance was not bad luck but the expected case.

  **One of the 21 was a real defect and is fixed**: chapter 13's review question
  used `đầu vào ngoài phân phối`, chapter 08's term, exactly once, wrapped inside
  an indented `cauhoids` item, and never glossed it anywhere in the chapter, so
  the gate could not see the omission. The term is now on one line with its
  `\tn`. The other 20 are all terms the chapter glosses elsewhere, so nothing
  else shipped wrong.

  **And the proposed fix will surface at least one false positive, which the
  author should know before merging it.** Chapter 13's summary contains
  \enquote{sự đánh đổi có tốc độ / đo được}, where `độ` ends one line and `đo`
  begins the next; under an `\s+` pattern that matches chapter 06's term
  `độ đo` inside the ordinary phrase `tốc độ đo được`. It is invisible today only
  because the two words are split, and the string `tốc độ đo` occurs nowhere in
  the book on a single line. That is decision 31's and decision 50's false
  positive arriving from a new direction, and it is the argument for landing the
  fix and the reword together rather than the fix alone.
...
- **The chapters are running six to ten typeset pages each.** Re-measured on the
  ...
  **Chapter 14 completes Part IV, and every page figure above is now stale**,
  because the 2026-09-05 reading-flow pass added worked examples and maths
  ladders to chapters already drafted. Re-measured on the 2026-09-05 chapter 14
  build, chapters 01 to 15 start on printed pages 3, 9, 17, 23, 29, 37, 45, 57,
  67, 79, 89, 99, 113, 129 and 147. That gives Parts I and II 34 pages over five
  chapters, Part III 20 over two, and **Part IV 90 over seven**: 10, 12, 10, 10,
  14, 16 and 18. The core averages 12.9 pages against the 6.8 of Parts I and II,
  and chapter 14 is the longest chapter in the book. Read the per-chapter figures in the rows
  above as the numbers of the build that recorded them, not as current.

  **With all seven core chapters drafted, the arithmetic this item has been
  waiting for can be done on measurements rather than on an extrapolation.** The
  book stands at about 183 printed pages including front and back matter, of
  which the four undrafted chapters currently occupy about ten as stubs. Those
  four are 15 through 18, and the preface is still to come. At Part IV's rate
  they add roughly 52 pages and land the book near 225; at Parts I and II's
  rate, roughly 27, landing near 200. **Both ends of that range are under decision 6's 300 to
  400**, so the choice is no longer deferrable and it is the author's: revise
  decision 6 down to what the current density gives, or assign more papers to
  the four remaining chapters and say which. Chapter 14 is also evidence on the
  question itself, and it cuts against paper count as the lever: it read three
  corpus papers and one outside them, more than any chapter so far, and came out
  the same length as chapter 13, which read one. The evidence is complete; only
  the call is missing.

  **Chapter 15 is the first data point outside Part IV and it lands at 16 printed
  pages, 147 to 162**, against Part IV's 10, 12, 10, 10, 14, 16 and 18. So the
  first chapter of Part V is written at the core's density and not at Parts I
  and II's, and it read three corpus papers plus one outside, the same load as
  chapter 14. Re-measured on the 2026-09-06 build, the book stands at 208 PDF
  pages, of which the three chapters still stubbed take about seven. **At
  chapter 15's rate those three add roughly 48 pages and land the book near
  250 with the preface**; at Parts I and II's rate, near 220. Both ends are still
  under decision 6's 300 to 400 and the gap has narrowed by about 25 pages since
  chapter 14's estimate. The call is unchanged and it is still the author's, but
  the arithmetic now has a Part V measurement in it rather than an extrapolation
  from Part IV alone.

  **Chapter 16 is the second Part V data point and it is the one that separates
  the two candidate levers for good.** It runs 13 printed pages, 163 to 175,
  against chapter 15's 16 and Part IV's 10, 12, 10, 10, 14, 16 and 18. And it
  reads **no new paper at all**: its four papers were each close-read by an
  earlier chapter. Every earlier reading of this item has debated whether the
  lever is the number of papers or the depth of their material; chapter 16 shows
  it is neither on its own, because a chapter with zero new papers lands inside
  the range of chapters with one and with three. **What sets the length is how
  much the chapter has to say, and a synthesis chapter has as much to say as the
  chapters it synthesises left unresolved.** Re-measured on the 2026-09-06 build,
  the book stands at 221 PDF pages with two chapters still stubbed. At chapter
  16's rate those two add roughly 26 pages and land the book near 250 with the
  preface; at Part IV's rate, near 250 as well, because the two estimates have
  now converged. **The range has collapsed to a point and it is still under
  decision 6's 300 to 400**, so the call this item has asked for five times is
  now a call about one number rather than about a range: either revise decision
  6 to about 250, or say which of chapters 17 and 18 gets more papers and how
  many. It is still the author's, and it is now the only thing standing between
  this item and closing.
