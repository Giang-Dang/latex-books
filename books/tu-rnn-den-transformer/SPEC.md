...
Chapter 11, "Ảnh là 16x16 chữ", is drafted at companion tag `ch11`. It uses
the chapter 10 CIFAR-10 recipe to test only the small-data, laptop-scale side
of the ViT claim; the ImageNet/JFT result remains a citation rather than a
reproduction. Its train diagnostic is explicitly limited to the first 10,000
training examples (decision 79). Next action: chapter 12, after re-verifying
its 2021--2026 facts.
...
Rows 78 onward were written in the chapter 11 session, 2026-08-13.
...
11. **Ảnh là 16x16 chữ** (Dosovitskiy et al. 2021) - patch embedding, the `CLS`
    ...
    trained on CIFAR-10 under chapter 10's recipe tests the same small-data
    condition at laptop scale, not the ImageNet/JFT result; gap: data-hungry,
...
| 11 | drafted | Companion tag `ch11`, module `vision.py`, test `tests/test_ch11.py` and one experiment script. The ViT has 66,095 parameters against chapter 10's CNN at 66,570: 8x8 RGB patch embeddings, a learned class token and positional embeddings, one post-LN encoder layer, final LayerNorm and linear head. Its 12 new trainings reuse chapter 10's four data sizes, epochs, Adam recipe, subset standardization and seeds; CNN and MLP controls are the committed unmodified `ch10` canonical run rather than a second training of the same code. On CIFAR-10 the ViT test accuracy is 0.2948 / 0.3578 / 0.4150 / 0.4576 across 1,000 / 4,000 / 16,000 / 50,000 images. Its diagnostic train accuracy, scored on at most the first 10,000 training images, is 0.5297 / 0.4306 / 0.4418 / 0.4644, so the chapter does not call the gap a generalization-only result. The paper result is cited, not reproduced: table 5's ViT-B/16 goes 77.91 to 84.15 when pretraining changes from ImageNet-1k to JFT-300M at fixed 384 fine-tuning. One TikZ figure, measured 302.15pt against a 441.02pt measure. Decision 78 defines the architecture and decision 79 scopes the diagnostic |
...
- **Economy:** `chặt`, which is what `tone-chuong-sach.md` sets and this book
  ...
  `chặt` for their own reasons; see decision 81. This governs prose written from
