...
tầng, mỗi tầng một nghìn cell, nên \enquote{the deep LSTM uses 8000 real numbers
...
Không phải quy ước cho gọn. Bài nêu đúng lý do: nó \enquote{enables the model to
define a distribution over sequences of all possible lengths}. Không có nó thì
...
đích, trọng số riêng biệt. Lý do bài đưa ra là nó tăng số tham số
\enquote{at negligible computational cost} và làm cho việc huấn luyện nhiều cặp
ngôn ngữ cùng lúc trở nên tự nhiên. Trong repo companion, đó là hai module
\code{encoder} và \code{decoder} độc lập nhau.
...
Ông cũng nói thẳng chỗ mà mục~\ref{sec:ch04-cat-gradient} đã mổ: bản gốc 1997
...
nói ra đây thay vì để listing ngầm bảo rằng bài đơn giản hơn nó thật sự.
...
\begin{figure}[htbp]
  \centering
  \input{figures/tikz/ch05-encoder-decoder}
  \caption{Encoder-decoder, vẽ theo hình 1 của bài. Encoder đọc câu nguồn đã
    ...
    nối thêm đường thứ hai qua chỗ thắt này.}
