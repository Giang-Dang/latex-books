...
Bài viết thẳng rằng các tín hiệu cập nhật sẽ đẩy \(w_{ji}\) vào cả hai việc
cùng lúc (\enquote{will attempt to make \(w_{ji}\) participate in (1) storing
the input (by switching on \(j\)) and (2) protecting the input (by preventing
\(j\) from being switched off by irrelevant later
inputs)})~\autocite{hochreiter1997lstm}. Hai việc, một con số.
