...
vào $\vect{x}_t \in \R^{d_x}$, giữ một \tn{trạng thái ẩn}{hidden state}
$\vect{h}_t \in \R^{d_h}$,
...
bias $\vect{b}_h$, rồi đẩy qua hàm $\tanh$. Kết quả là trạng thái
...
\eqref{eq:rnn-hidden}. Nhưng với một \tn{chuỗi}{sequence} có độ dài $T$ cố
định, ta có thể
...
dụng đúng thuật toán \tn{lan truyền ngược}{backpropagation} thông thường; chỉ
khác là gradient
...
Cuốn sách này làm việc với gradient ở dạng
\tn{\textbf{ma trận Jacobi}}{Jacobian matrix}. Nếu
