...
Cách duy nhất để biết chắc một cài đặt
\tn{lan truyền ngược}{backpropagation} là đúng: so sánh nó
với gradient được tính bằng
\tn{\textbf{sai phân hữu hạn}}{finite difference}. Đây là kỹ thuật lâu đời
...
Với một tham số vô hướng $\theta$ và một \tn{hàm mất mát}{loss function}
$L(\theta)$, đạo hàm
được xấp xỉ bằng:
...
Với một \tn{ma trận trọng số}{weight matrix} $\mat{W}$, ta áp dụng
\eqref{eq:finite-diff} cho
từng phần tử một. Nếu $\mat{W}$ có $m \times n$ phần tử, ta cần $2mn$ lần
chạy lan truyền xuôi. Với RNN đồ chơi $3 \times 2$ của tôi, đó là 12
