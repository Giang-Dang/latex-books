...
client discovers at run time, which is why a query against \code{node} that
wants more than the id carries either \code{\_\_typename} or an inline
fragment. An inline fragment is the \code{... on Session \{ title \}} form,
and it selects its fields only when the object turns out to be that type; the
next subsection sends one.
