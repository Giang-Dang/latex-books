...
book has composed with since chapter~\ref{ch:composition}. Its reference page
says what it does~\autocite{cosmoclicheck}: hand it one subgraph document as a
path, and it checks for breaking changes and composition errors in every
federated graph that subgraph belongs to.

\index{schema check!needs a control plane}%
One document in, every graph it belongs to checked. Knowing which graphs those
are, and what schema each of them is currently serving, is what a control plane
is for, and the CLI reference documents how the command reaches one: an
environment variable, \code{COSMO\_API\_KEY}, which for Cosmo Cloud arrives with
the account~\autocite{cosmocli}. The same holds for \code{wgc proposal create},
for \code{wgc subgraph fix} and for \code{wgc federated-graph check}. That is
not a missing flag. It is the shape of the question.

\index{schema check!compose is the exception}%
\code{wgc router compose} has been the exception all along, and its own
reference page carries the sentence this book has been quietly relying on since
chapter~\ref{ch:composition}: it composes locally, without a connection to the
control plane, and does not interact with one at all~\autocite{cosmocompose}.
...
for that subgraph, across every graph that subgraph is part of, which is why
the reference page counts composition errors in the plural. The second is the
one nothing local can do. It takes the changes the first found and asks whether
any client actually used them.

\index{schema check!four components, and the verdict}%
Cosmo's documentation divides a check into four named components rather than
two comparisons, and the two counts do not disagree: composition errors and
breaking change detection are what the diff produces, the operations check is
the second comparison, and a lint check runs beside both. The verdict is
narrower than that list. A check passes when the proposed schema was both
composable and non-breaking, and fails when it was either not composable,
introduced breaking changes, or both~\autocite{cosmochecks}. The sentence that
says so names two of the four.
...
own: \code{--skip-traffic-check}, which the reference page describes as
skipping the check against client traffic, after which any breaking change
fails the run~\autocite{cosmoclicheck}. That is
...
\subsection{What Having One Costs}

\index{control plane!the two ways to have one}%
There are two ways to have a control plane behind that command and they are
priced nothing alike. The managed one is an account, and the API key arrives
with it. The other is Cosmo's own, which is open source and can be self-hosted,
and what self-hosting means in WunderGraph's documentation is Docker Compose on
a laptop or Kubernetes with a Helm chart in production, standing up a Node
service, PostgreSQL, Redis, ClickHouse, an S3-compatible object store, Keycloak
with a PostgreSQL of its own, a CDN server, an OpenTelemetry collector and a
metrics collector~\autocite{cosmoselfhosted}.

\index{control plane!what a graph without one has instead}%
That is the bill for the second comparison, and it is worth reading against
what it buys. The graph in this book is four processes, four SQLite files and
one downloaded binary, with no container in it anywhere. Nothing in that list
records what a client asked for last Tuesday, and no amount of composing will
make it. The record is the product.
...
section~\ref{sec:ch16-the-change-this-book-already-shipped} is it. Compose the
four documents you have published, then the four you are about to publish. It
is the command from chapter~\ref{ch:composition}, run twice with the documents
edited in between:

\begin{minted}{text}
wgc router compose -i graph.yaml -o published.json
wgc router compose -i graph.yaml -o candidate.json
\end{minted}

\code{engineConfig.graphqlSchema} is where each config carries the schema a
client reads. Pull it out of both and diff them, and the report is the five
changed lines that section prints.
...
subgraph document and nothing else~\autocite{cosmoclicheck}. It can, because a
control plane is holding
...
What that does not give you is the second comparison, and the shortfall is
exact. The diff tells you which of the sixteen
...
\begin{figure}[htbp]
  \centering
  \input{figures/tikz/ch16-what-each-check-can-see}
...
A breaking change is the largest availability risk a federated graph carries,
larger than a subgraph outage and larger than anything in
chapter~\ref{ch:blame-routing}. That is my judgment and it cites nobody: no one
running a federated graph at scale has published the outage that would settle
it. The reasoning is the one this chapter measured. An outage ends when the
service comes back, and
...
catches it, so take that as agreement rather than as evidence.
