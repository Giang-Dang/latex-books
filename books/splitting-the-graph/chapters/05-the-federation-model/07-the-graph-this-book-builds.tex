...
Chapter~\ref{ch:first-subgraph} turns the service you already have into the
first of those three documents: the package, the key, the reference resolver,
and the two fields the contract adds to \code{Query}. It ships one subgraph and
composes nothing, because a graph of one is still a graph and it is the smallest
