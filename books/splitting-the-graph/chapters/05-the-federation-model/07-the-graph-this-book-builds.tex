...
Three services to begin with, and the split is the obvious one: whoever stores
the rows owns the type. Sessions keeps what chapter~\ref{ch:schema-design} left
it. Speakers takes the other table. Ratings is new, and it exists because a
service that contributes to a type it does not own is the arrangement this
model is for, and a two-service example never shows it. A fourth arrives in
chapter~\ref{ch:cross-seam-paging}, for the reason this chapter closes on.
...
And Ratings declares only a type it does not own, because everything it
stores is about that type:
...
are answered by a service which has never heard of a conference schedule, and
nothing will say so about the third that service adds in
chapter~\ref{ch:second-third-subgraph} either. The
...
Chapter~\ref{ch:enter-the-router} puts a router in front of that one subgraph,
which is the only arrangement in which you can watch what a router does with no
seam confusing the reading. Chapter~\ref{ch:second-third-subgraph} adds the
other two services, the directives in
section~\ref{sec:ch05-the-directives}'s seam group and the first query answered
across two subgraphs, and ends with the graph part~III spends nine chapters
breaking.
\index{conference example!split into three|)}%
