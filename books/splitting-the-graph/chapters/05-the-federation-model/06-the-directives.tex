...
Chapter~\ref{ch:second-third-subgraph} is where all three get used on a real
seam, and chapter~\ref{ch:satisfiability} is where getting them wrong produces
...
state a requirement in its schema instead of in its resolvers, so the router
can refuse a request before any subgraph is asked. Chapter~\ref{ch:entities-authorization}
