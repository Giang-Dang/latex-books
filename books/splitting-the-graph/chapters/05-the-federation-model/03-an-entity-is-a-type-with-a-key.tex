...
That is a stub: one field, there so that the value can be written down. This is the
...
same way. The book already has its keys. Using them as federation keys is my
judgment rather than anything Apollo publishes, which says nothing either way
about a global id as a key, and chapter~\ref{ch:second-third-subgraph} presents
...
\index{extend@\code{extend}!definition}%
\code{extend} is GraphQL's own keyword for adding fields to a type declared
elsewhere in the document, which is how a library contributes to a
\code{Query} it does not own.
