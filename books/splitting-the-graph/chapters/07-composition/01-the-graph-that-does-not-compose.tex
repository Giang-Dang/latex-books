...
That installs whatever is current. The version table in
appendix~\ref{app:toolchain} pins the release this book's error text was
captured with, and a later one may word an error differently, so add \code{@}
and that number to the command if you want the text on these pages word for
word.
...
go beside it; \code{graph} for what the composer reads and the one file it
writes; and \code{router} for the one file chapter~\ref{ch:enter-the-router}
adds. Here is the whole of the input, for a
