...
composer that this subgraph declares the key so that its own fields can hang
off the type, and that nothing should try to enter through it. That is the
right thing to say about a type you only point at, and the wrong thing to say
about a type you contribute fields to, because those fields can then be
reached from nowhere. Ratings also exports no \code{node} field, so there is
no other door.
