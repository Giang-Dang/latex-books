...
example exists to show. The mistake is the argument on the key.
\code{resolvable: false} tells the composer that this subgraph declares the key
so that its own fields can hang off the type, and that nothing should try to
enter through it. Ratings also exports no \code{node} field, so there is no
other door.
...
\begin{figure}
  \centering
  \input{figures/tikz/ch07-the-walk}
...
including the case that makes them hard: a field reachable several ways, none
of which work, and a composer that names one of them.
\index{resolvability!is a second pass|)}%
