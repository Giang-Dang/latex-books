...
\index{supergraph}%
\index{router}%
Each service publishes its own schema, and a service that does so is a
subgraph. A composer merges those schemas into one supergraph. A router serves
the supergraph and calls the subgraphs to answer each part of a query.
