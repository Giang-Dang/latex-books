...
\subsection{The Projection Is Left Behind}

\index{QueryContext@\code{QueryContext<T>}!not taken by the reference resolver}%
\index{projection!not taken by the entity route}%
One parameter is missing from the resolver below, and it is the one
chapter~\ref{ch:data-without-n-plus-1} put on every resolver that goes to a
table. A \code{QueryContext} can be injected into a reference resolver, and I
am leaving it out on purpose: chapter~\ref{ch:router-n-plus-1} measures what
this route pays for taking one, and
section~\ref{sec:ch06-what-the-entity-route-costs} measures what it pays for
not. The comment in the resolver says the parameter was left out rather than
forgotten, and names the chapter that says why.
...
loader, because that is what its node resolver does, and it takes no
\code{QueryContext}, for the reason above:

\begin{minted}[highlightlines={2-4,10-12,38-60}]{csharp}
...
public static partial class SessionType
{
    ...
    /// undecoded node id string, so this method decodes it. And no
    /// QueryContext is taken, so every column is selected; one can be
    /// injected here, and chapter 10 measures why this route takes a
    /// batch instead. One statement per representation is what that
    /// chapter fixes.
    /// </summary>
    [GraphQLIgnore]
...
That compiles, and the entity route answers. The operation is short and the
