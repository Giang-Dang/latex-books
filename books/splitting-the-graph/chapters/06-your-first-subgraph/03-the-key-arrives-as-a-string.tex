...
One parameter is missing from the resolver below, and it is the one every
other resolver in this book takes. A \code{QueryContext} can be injected into a
reference resolver, and I am leaving it out on purpose. The reason is a
measurement rather than a rule: this route has to answer a list of keys, and
chapter~\ref{ch:router-n-plus-1} measures why a resolver that takes the
projection cannot also answer that list in one statement. That chapter weighs
the two and takes the batch. For now the consequence is enough.
...
comment in the resolver says so, rather than leaving a later reader to wonder
whether the parameter was forgotten.
