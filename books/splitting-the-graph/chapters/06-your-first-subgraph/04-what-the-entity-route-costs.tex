...
section~\ref{sec:ch06-the-key-arrives-as-a-string} could not give the reference
resolver a \code{QueryContext}, so the projection
chapter~\ref{ch:data-without-n-plus-1} built has no way to reach it. Six columns
...
\begin{figure}
  \centering
  \input{figures/tikz/ch06-two-routes}
  ...
  one column. Lane~B is given a string, decodes it itself, has no projection to
  be given, and asks for six. The braces mark where the decoding happens, and
...
differences: what the resolver is handed, what it may inject, and what it costs.
