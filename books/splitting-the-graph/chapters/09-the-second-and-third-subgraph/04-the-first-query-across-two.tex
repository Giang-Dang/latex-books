...
\begin{figure}[htbp]
  \centering
  \input{figures/tikz/ch09-two-fetches}
...
HTTP 200, every title present, every speaker null, and five errors. The first
names both the subgraph that failed and the path it failed at, and that path
is new. When there was one subgraph the error could only be about the whole
request; with a seam it can say which field position the hole is in, which is
the difference between a page that says \enquote{the graph is down} and a page
that can render the schedule and gray out the names.

\index{partial failure!one propagation error per row}%
The other four are one per session, and they are
chapter~\ref{ch:null-blast-radius}'s subject arriving early.
\code{Speaker.name} is \code{String!}, the router has no speaker to put a name
on, and each row reports the null it cannot hold before the failure climbs to
\code{speaker}, which can hold one and does. Four rows, four reports, and the
titles beside them untouched.
