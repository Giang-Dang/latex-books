...
specification section moves after the directive tour (decision 129), chapter 1
says what it assumes and eight terms get a clause at first use (decision 130),
...
| 130 | The reader is told what the book assumes, and a term gets one clause at first use | Chapter 1 gains a closing subsection naming three prerequisites: C# and ASP.NET Core at the level of a shipped controller, HTTP and JSON, and GraphQL at the level of one query sent and one schema read. Eight terms were in use before any gloss: resolver (2.3), selection set and the connection shape (3.5), `__typename` (4.1), inline fragment, introspection and variables (4.3) and `extend` (5.3). Each now gets one clause in the sentence that uses it, and an index entry, so the chapter 1 promise that the index carries every one of them holds. The connection paragraph cites the Relay cursor connections specification, `relayconnections`, unsigned under decision 39, and says that `nodes` is Hot Chocolate's and not the specification's, which was checked by reading the document. **The rule from here:** a term a reader at that level may not have met gets one clause, in the sentence, never a paragraph of theory ahead of the behaviour; decision 14 still governs. |
...
- **Terms:** chapter 1 names what the reader is assumed to know, and a term a
