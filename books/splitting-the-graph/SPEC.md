...
A reading-flow pass on 2026-09-05 read chapters 1 to 14 end to end as a junior
developer new to GraphQL would, and found the confusion in three places: claims
an early chapter makes that a later one reverses, vocabulary the book never
defines, and things a build-along reader has to type that no page shows. The
worst was chapter 6 teaching that a `QueryContext` cannot be injected into a
reference resolver, which decision 96 had already found false and left standing
at five tags; the tags are amended and the chapter reprinted (decision 127).
Chapter 9's trimmed response is printed whole (decision 128), chapter 5's
specification section moves after the directive tour (decision 129), chapter 2's
opener says what the book assumes and eight terms get a clause at first use
(decision 130),
chapter 13's prose is brought into the book's voice with its listings untouched
(decision 131), and `wgc` is finally installed on the page (decision 132). No
count changed, and the verification repo's tags for chapters 6 to 9 are the
only thing outside `books/` the pass touched.

Chapter 15 opened on a mistake the book leaves available: chapter 7 warns in one
sentence that `resolvable: false` is wrong on a type you contribute fields to
(decision 132) and chapter 9 uses it correctly once, on a stub, which is not
much to hold an argument that reads in English as *this service does not own
these rows*. The Ratings service does not own `Session` rows either. Put it on
that key and the
...
of the mistake** (decision 134): removing a service that declares no `Session`
...
all** (decision 135): the same broken document composes silently in the
...
rejected option turns out to buy the check back (decision 136). `verify.ps1` is
...
| 127 | Chapter 6's listing comment is corrected at every tag that prints it, and the tags moved | Closes decision 96's residual, on the author's call. The doc comment on `ResolveSessionReferenceAsync` saying a `QueryContext` cannot be injected sat in `src/Sessions/SessionType.cs` at ten tags: `ch06-unguarded`, `ch06`, `ch07-costly`, `ch07-unshared`, `ch07-typelevel`, `ch07`, `ch08-noplan`, `ch08`, `ch09` and `ch09-noserializer`. The `hc14` tags never carried it. Each got one new commit on top of the tagged commit, rewriting the comment and nothing else, and the tag moved onto it, along with the side branch of the same name where one exists; `main`'s history is untouched, because chapter 10 already rewrote the comment there. `verify.ps1` was re-run at `ch06`, `ch07`, `ch08` and `ch09`, PASS at each. Chapter 6 reprints both `SessionType.cs` listings from the amended tag, sections 6.3 and 6.4 and the summary bullet say the resolver is given no `QueryContext` and that chapter 10 measures why, and chapter 9.5 no longer names chapter 6 as a cause of `Unexpected Execution Error`. **One trap found on the way, worth more than the row:** `verify.ps1` strips the box `wgc` draws around a composition error with a regex over the box-drawing characters, and a `pwsh` launched from a shell whose console is not UTF-8 decodes those bytes under the OEM code page, so two chapter 7 assertions fail on a graph that is fine. Force `[Console]::OutputEncoding` to UTF-8 before running the script from anywhere but an interactive console. |
| 128 | Chapter 9's half-down response is printed whole | Closes the open item decision 125 left, the way that decision leaned. The listing in 9.4 carries all five errors, re-captured at tag `ch09` on 2026-09-05 with the procedure in the chapter 9 note, and the paragraph under it says why there are five: one fetch failure, then one propagation error per row because `Speaker.name` is `String!` and the climb stops at the nullable `speaker`. Chapter 14.1 no longer opens by correcting chapter 9 and states the count as a fact of that page. The comment beside the count assertion in `verify.ps1` still describes the correction, which is history rather than a claim about the page, and stays. |
| 129 | Chapter 5's specification section moves after the directive tour | The section on why the book builds on Apollo Federation v2 rather than the Composite Schemas draft sat between the definition of a subgraph and the definition of an entity, where a reader meeting federation for the first time hits two specifications, a rewrite of Fusion and a version-support argument before they have met a key. It now follows the directive tour, which is where the TOC line always listed it. Section files renumbered with `git mv` so filesystem order matches book order; labels are unchanged, so every cross-reference resolves. Rejected: leaving the order and adding a skip note, which papers over an arc problem with a signpost. |
| 130 | The reader is told what the book assumes, and a term gets one clause at first use | Chapter 2's opener gains a paragraph naming three prerequisites: C# and ASP.NET Core at the level of a shipped controller, HTTP and JSON, and GraphQL at the level of one query sent and one schema read. It was drafted as a closing subsection of chapter 1 and the cold audit moved it: chapter 1 ends on "come back when a second team needs to change your schema and cannot", and a prerequisites list after that line is a chapter closing on a list rather than on its finding. Chapter 2 is where the reader starts typing, so it is where the assumption belongs. Eight terms were in use before any gloss: resolver (2.3), selection set and the connection shape (3.5), `__typename` (4.1), inline fragment, introspection and variables (4.3) and `extend` (5.3). Each now gets one clause in the sentence that uses it, and an index entry, so the chapter 2 promise that the index carries every one of them holds. The one exception to the clause is the connection, which is a shape rather than a word and gets a short paragraph, placed after the SDL block that shows it so that decision 14 holds. That paragraph cites the Relay cursor connections specification, `relayconnections`, unsigned under decision 39, and says that `nodes` is Hot Chocolate's and not the specification's, which was checked by reading the document. **The rule from here:** a term a reader at that level may not have met gets one clause, in the sentence, never a paragraph of theory ahead of the behaviour; decision 14 still governs. |
| 131 | Chapter 13's prose is brought into the book's voice, and its listings are not touched | The pass found chapter 13 in a different register from chapters 9 to 12: runs of uniform short sentences, *list owner*, *cut* and *hydration* used before they were defined, an opener that did not say what the chapter establishes in the reader's terms, no sentence tying a section back to what an earlier chapter measured, and `AsNoTracking` and the undocumented records passing without a word. Rewritten under the humanizer chapter profile with chapter 12's section files as the voice specification. Every listing is byte-identical to tag `ch13` and `check-listings.ps1` proves it. The doc-comment gap is stated in the prose rather than fixed in the source, because fixing it means amending `ch13` and `ch14` for schema descriptions that are not what the chapter is about; a later pass that amends those tags for another reason should add them. |
| 132 | Four smaller corrections from the same pass | `wgc` is installed on the page in 7.1, as `npm install -g wgc` with the version left to appendix A under decision 35, so chapter 8's "chapter 7 added `wgc` through npm" is now true; the `src/`, `graph/` and `router/` tree is stated once in 7.1 and `src/` is named in 2.1, which is decision 43's rule reaching the reader. Chapter 5's Ratings map, in 5.4 and 5.7, shows `averageScore` and `ratingCount`, which chapter 9 builds, in place of a `Rating` type and a `ratings` list it never does. Chapter 5.5's `@authenticated` sentence no longer says the router refuses before any subgraph is asked, which chapter 12 measured to be false (decision 109). Chapters 2.4 and 3.1 point their `@cost` forward references at chapter 7, which removes the defaults, ahead of chapter 20. Chapter 7.5 says the `resolvable: false` mistake is one of placement, since chapter 9 uses the same argument correctly on a stub. |
| 133 | The pass's own cold audit, and what it found | Two auditors read the changed chapters with no drafting context. Accepted: 6.3 still said the reference resolver "cannot have" a `QueryContext` one paragraph after saying it can; 10.4 and 10.5 still corrected a chapter 6 claim that chapter 6 no longer makes; 6.3 called the projection "the one every other resolver in this book takes", which four resolvers do not; the chapter 6 note still recorded the injection failure as a finding, and now marks it superseded; chapter 1.2 never used the word *subgraph* it indexed, and 1.3 used *key* and *entity* with no clause; 5.3 stated the node id as the book's key without the first-person marker decision 67 requires, which 9.8 claimed it had; 5.4 and 5.7 said Ratings contributes two fields where chapter 9 gives it three, and 5.5 said all three seam directives get used where `@provides` does not; 5.7 said three services where chapter 13 makes four; 5.3's *stub* was indexed on a paragraph that never used the word; the chapter 5 note's scratch composition predates the map change and now says so. Rejected on the record: 5.7's `Speaker` stub printed with a bare `@key`, because decision 85 settled that the map draws the state before the extraction; the amended listing comment's "chapter 10 measures why this route takes a batch instead", because the sentence is about chapter 10's measurement and re-amending ten tags for a tense is not a trade worth making; 5.1's "the answer chapter 1 spent a chapter arguing against", which predates the pass and is arguable rather than wrong; and the `@link` behaviour claims in 5.5, which predate the pass and are the chapter 5 note's to settle. **From the second auditor, accepted:** 13.2 named `sessions.nodes.@.averageScore` as an entity fetch path, which no note records and which the book's own captures contradict (an entity fetch is addressed at the object position), so the path is gone; 7.1 said `graph/` holds nothing generated when the compose command five lines below writes `router.json` into it, and told the reader to install a pinned release with an unpinned command, both reworded; 7.5's gloss of `resolvable: false` described the contributing case it then called wrong; 4.3 said "the subsection after next" where the next one sends the fragment, and "every query against `node`" where `{ node(id:) { id } }` needs neither; four index lines sat mid-paragraph and one used an unquoted `@` as its sort key; chapter 13's summary box was still in the old register and its symptom was a mechanism rather than what a reader would search for; 13.3 generalised about "federated graphs" and misquoted 9.7's reason for shipping no `@provides`; 3.5 claimed the connection is "the shape most GraphQL paging takes" with no source, and its paragraph explained the shape before the SDL showed it, so the claim is gone and the paragraph now follows the block; the chapter 13 opener announced its three moves; *list owner*, *read model* and the second sense of *projection* were used before a clause; one negative parallelism and one doubled "the whole" closer. **Rejected on the record:** 13.2's plan described rather than printed, which predates the pass; 9.4's "this exact request", which is the chapter 3 baseline the SPEC status names; 14.1's "three non-null fields cross a seam", which is correct as scoped; 4.3's uncited sentence about plural root fields, 3.5's forward announcement and 7.5's figure placement, all of which predate the pass and none of which a reading-flow pass owns. |


| 134 | The resolvability check answers per field, names one route, and the route it names is not a property of the mistake | Measured 2026-09-05 on `wgc` 0.129.9. One unresolvable field produces one error carrying one path; three contributed fields produce three errors, not one per failing route. Six routes reach `Session.averageScore` in this book's supergraph (`sessions.nodes`, `sessions.edges.node`, `sessionById`, `node`, `nodes`, and the `rescheduleSession` payload), eight once Search's two are counted, and all of them fail identically. **Which one is printed moves with the rest of the graph.** The four-service graph names `searchSessions.edges.node.session`; drop Search and the same defect is reported at `Query.node`, which is the error chapter 7 printed. Reverse the four subgraph entries in `graph.yaml` and the printed path holds while the third reason line names `search` where it named `sessions`, because both services declare a `Session` that cannot reach ratings and the composer names whichever it visited first. `wgc`'s own fixed 120-column box then elides the word `in` out of that sentence, and the command offers no structured output to read instead. Two runs of one input are byte-identical, so this is sensitivity rather than nondeterminism. **The scope line and chapter 7's forward reference both said four**; both are corrected in this session, chapter 7's to a count-free sentence rather than to a new number, because the number was never measured and a forward reference does not need one. |
| 135 | A composition that passes is not proof that a route exists, and a shareable `node` is what defeats the check | The finding that outranks chapter 15. Measured 2026-09-05: the same broken Ratings document that fails in the four-service graph **composes at exit zero, silently**, in the three-service graph chapter 9 finished with. The config it writes carries both halves openly, `disableEntityResolver: true` beside three fields the router is told that service resolves. Isolated one variant at a time, what buys the silence is a second subgraph declaring the singular `node(id: ID!): Node`: the plural `nodes` returning `[Node]` does not, implementing `Node` without declaring the field does not, and declaring the field while `Speaker` implements nothing at all **does**, with the composer warning that the interface has no implementation and then composing anyway. Read out of `@wundergraph/composition` 0.63.3: the walk records an entity when it is *visited* rather than when it resolves, under a code comment asserting the opposite; a shared root field is an OR across subgraphs, so Speakers answering for `Speaker` passes the field while `Session` stays in the record unresolved; and every later root field reaching it is skipped. That reading predicted that moving `node` and `nodes` to the bottom of `type Query` in the Sessions document would make the same graph fail, and it does, naming `Query.sessions`, with the healthy control still composing. **At run time the approved graph produces three different failures from one defect**: `averageScore` answers HTTP 500 on all six routes, the first 500 in this book; `ratingCount` answers 200 while the router sends `{ ratingCount }`, the root field, at path `sessions.nodes`, because a root field of that name exists to fall back to; and `feedbackUrl` answers 200 with four nulls and **no `errors` key**, because `@requires` makes the router plan a `BatchEntity` fetch against a service whose schema has no `_entities`. `ch15-falsepass` carries it. **Not established:** whether abstract return types in general do this rather than `Node` specifically, and whether anyone has reported it upstream; neither is claimed. |
| 136 | Decision 90's second rejected option is not neutral, and it buys the check back | Chapter 9 rejected four ways of making `node(id:)` route across a seam and recorded that only the first was certain. The second, declare `node` in one subgraph only, was rejected on the reasoning that composition would pass and the failure would be identical. Measured 2026-09-05: take `node` and `nodes` out of the Speakers document and the healthy four-service graph still composes at exit zero, while the broken one fails again and names `Query.node`. So the option costs nothing at composition time and removes exactly the condition decision 128 turns on. **The runtime half was not measured** and the chapter says so: with one subgraph declaring the field, a `node(id:)` for a speaker must be planned through the Sessions subgraph's copy and what happens then is unknown. The chapter recommends trying it before sharing the field, as my judgment under decision 38, and states the untested half rather than rounding it off. |
| 137 | Chapter 15 issues no HotChocolate 14 callout, deliberately | The sixth chapter to decline one, after decisions 64, 91, 98, 103, 111 and 118, and the reasoning is decision 71's. Everything this chapter measures needs two subgraphs both declaring `Query.node` and a third contributing fields to a type it does not own. On `hc14` there is one subgraph, `Session` is not an entity at all, and the composer never reaches the resolvability pass for that input because the merge fails first, which decision 65's neighbourhood already recorded and `ch07-hc14` already asserts. A callout would restate that. `ch15-hc14` should tag an unchanged tree at 216, on the same footing as `ch05`, `ch08` and the four that followed (decision 66), and **it has not been run or tagged yet**: the `hc14` branch is checked out in a leftover worktree from the chapter 13 session, at `.claude/worktrees/stg-graph-ch13-hc14`, which this session did not modify because it is another session's working state. What is established without the run is that the chapter 15 block cannot execute there at all: it is gated on `SearchDomain` and `NullableAcrossSeam`, both false on that branch. Recorded here rather than left as a silent gap, because a progress row claiming a PASS that nobody ran is the thing this book's verification discipline exists to prevent. |
| 138 | This composer does not require a `@requires` dependency to carry the same access control, and what protects the graph is the subgraph | Measured 2026-09-05, and the reason the open item pointing chapter 15 at CVE-2025-64172 is now narrower rather than open. That advisory is `@apollo/composition` not requiring a field using `@requires` to carry the access control of the field it depends on. `wgc` 0.129.9 does not require it either: a graph whose `Session.title` is `@authenticated` and whose `feedbackUrl` is `@requires(fields: "title")` composes at exit zero and the composer writes one `authorizationConfiguration`, on `title`, and none on `feedbackUrl`. On the book's own graph with `Session.Title` guarded, the same holds: two rules, `Session.title` and chapter 12's `Speaker.email`, and none on the field built out of the first. **At run time the guard holds anyway**, and not because of anything composition or the router did: the refusal arrives with its path naming `title` on a request that never mentioned it, because the router must collect `title` from Sessions before Ratings can be asked and the Sessions service refuses that field to an anonymous caller. Under decision 109 the router enforces only the rules it is given, and it was given none. **Not measured, and it is the case that matters:** a graph guarded only at the router, chapter 12's `ch12-routeronly` shape, where no subgraph refuses anything. Searched four ways for any published advisory against `@wundergraph/composition` or `wundergraph/cosmo` and found none on any subject, which is an absence of advisories rather than of defects. None of this reaches chapter 15's prose: the chapter's one sentence is about what a resolvability answer means, and a transitive-authorization finding is chapter 12's subject arriving in the wrong chapter. `verify.ps1` asserts the composition half; `ch15-requiresguard` is an experiment branch and carries no tag. |

| 139 | A change the book argues against and does not ship may be stated in prose, because a fragment is what decision 16 forbids and prose is not a listing | Raised by chapter 15's cold audit, which read the chapter changing one attribute in `src/Ratings/SessionType.cs` and never reprinting the file, and called it decision 16's fragment rule. The finding is rejected and the reasoning is recorded rather than left to be re-derived. Decision 15 is about **listings**: a file appears in full the first time it matters, and decision 16 forbids showing a later change as anything smaller than the whole file or the complete enclosing member, because three lines of context is a fragment wearing a diff's clothes. Chapter 15 prints no listing of that file at all. It says, in a sentence, to put `resolvable: false` on the key and take the reference resolver off with it, which is complete: there is nothing a reader could type wrongly from it, and the enclosing member here is the whole 108-line class, reprinted to mark one attribute in a state the reader is being told not to adopt. Chapter 11 is not the counter-example it looks like: there the broken listing **was** the chapter's subject and a reader had to read it line by line, where here the subject is the composer's message and the C\# is its cause in one line. The rule this settles: a file the book has already printed, changed into a state the book argues against and never ships, may be described in prose provided the description is complete enough to reproduce and a tag carries the state; printing part of it stays forbidden. `ch15-falsepass` is the tag. |
...
2. **Hot Chocolate from Zero** - what the book assumes its reader knows, the first service, implementation-first types, the schema it produces, and the development loop
...
15. **Where Satisfiability Actually Fails** - reading composition errors, and why the composer names one route out of the several that fail. **Corrected 2026-09-05 while drafting.** The scope line said four; this book's graph offers six routes to a contributed field and eight once Search is in, and the count was a guess made before the chapter was measured. Chapter 7's forward reference carried the same number and is corrected in the same session (decision 134). Two clauses added rather than changed: which route is named moves with subgraphs that have nothing to do with the mistake and with the order of the lines in `graph.yaml` (decision 134), and the check can be satisfied without finding a route at all, by a shareable root field returning an interface (decision 135), which is the second bill for decision 74's `@shareable` on `Query.node` and the collection decision 90 asked chapter 15 for.
...
| 01 | drafted | No code, as planned. Note at `research/ch01-do-you-actually-need-this.md`. Established the summary box (decisions 20, 21, 41) and the figure idiom (decision 33); `figures/tikz/ch01-one-field.tex` is the idiom specification later figures are matched against. Its threshold is stated as my judgment under decision 38, because no source passed the Sources bar. Reading-flow pass 2026-09-05: *subgraph*, *key* and *entity* get a clause at first use, and the merge federation performs is separated from the merge requests 1.1 quotes (decisions 130, 133). |
| 02 | drafted | Note at `research/ch02-hot-chocolate-from-zero.md`. Stood up the verification repo, tags `ch02` and `ch02-hc14`, `verify.ps1` PASS on both. Emitted the first callout, which fixed the `hc14` file set (decision 47) and settled decisions 42 to 46. Measured the listing column budget at 73. Reading-flow pass 2026-09-05: the opener names what the book assumes of its reader, *resolver* gets a clause at first use, `src/` is named, and the `@cost` forward reference points at chapter 7 (decisions 130, 132). |
| 03 | drafted | Note at `research/ch03-data-without-the-n-plus-1.md`. Four tags, `verify.ps1` PASS on each: `ch03` (main), `ch03-naive` and `ch03-starved` (the two states the chapter argues against, decision 51), `ch03-hc14`. The single-service baseline chapter 10 measures against is **5 statements naive, 2 batched**, for four sessions and three distinct speakers. Settled decisions 51 to 55. Turned `sessions` into a Relay connection, the first change to break an earlier chapter's printed requests. Reading-flow pass 2026-09-05: the connection shape is now explained and cited before its SDL appears (decision 130). |
...
| 05 | drafted | Note at `research/ch05-the-federation-model.md`. No C#: `git diff ch04..ch05` over `src/` is empty, and `ch05` is the book's first tag on an unchanged tree (decision 66). `verify.ps1` gained eight assertions for the request the chapter opens on and prints, 104 total, PASS on `main` and `hc14`. Confirmed decision 31's `graphqlsdl` environment against real federation SDL, which was this chapter's stated job. Paid chapter 1's debt on why Apollo Federation v2 rather than the Composite Schemas draft, and corrected two factual clauses in decisions 6 and 7 in the process. Settled decisions 63 to 66. Reading-flow pass 2026-09-05: the specification section moved after the directive tour, the Ratings map matches what chapter 9 builds, and the `@authenticated` sentence matches what chapter 12 measured (decisions 129, 132). |
| 06 | drafted | Note at `research/ch06-your-first-subgraph.md`. Four tags, `verify.ps1` PASS on each: `ch06` (main, 148 assertions), `ch06-unguarded` (9, the reference resolver that does not read the type name inside the key), `ch06-hc14-ignored` (125, the attribute idiom compiling and doing nothing on 14.3.1) and `ch06-hc14` (128, the code-first route that works there). The book's first federation C#, and its first version difference that is not a difference in output but in whether the code works at all. Settled decisions 67 to 72. Reading-flow pass 2026-09-05: the `QueryContext` claim corrected in prose, summary and both listings, with the tags amended (decision 127). |
| 07 | drafted | Note at `research/ch07-composition.md`. Five tags, `verify.ps1` PASS on each: `ch07` (main, 195 assertions), `ch07-costly` (14, chapter 6's subgraph unchanged, which does not compose), `ch07-unshared` (12, the cost fix in and the shareable fix out), `ch07-typelevel` (12, the one-line fix and the ownership check it switches off) and `ch07-hc14` (174). The book's first non-.NET tool, its first artifact defined only by a protobuf, and its first failure caused by two vendors disagreeing rather than by anybody's mistake. Paid the `Query.node` bill and re-pinned `wgc` to 0.129.9. Settled decisions 73 to 78. The audit found fifteen things worth fixing, including two claims the research note itself had marked as not established; they are listed in the pull request. Reading-flow pass 2026-09-05: `wgc` installed on the page and the project tree stated (decision 132). |
...
- **Terms:** chapter 2's opener names what the reader is assumed to know, and a term a
  reader at that level may not have met gets one clause in the sentence that
  first uses it, plus an index entry (decision 130). One clause, not a
  paragraph: decision 14 still bars theory ahead of the behaviour.
...
- **Whether anything can make `node(id:)` route correctly across a seam.**
  ...
  composition and is decision 136**: declaring `node` in one subgraph only
  composes the healthy graph unchanged and restores the check decision 135
...
- **Whether `@wundergraph/composition` has ever had the access-control bugs
  ...
  empty. **The transitive case is now tested and is decision 138**, so what is
...
- **Whether the check decision 135 defeats is defeated by abstract return
...
- **Whether anyone has reported decision 135 upstream.** No search was made,
