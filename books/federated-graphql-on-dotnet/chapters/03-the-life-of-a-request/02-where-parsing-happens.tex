...
rules against the server's own, and said that the two rejections happen in
different places. Now the pipeline is on the table I can say where. The unknown
field is validation, at position six, inside the pipeline. The syntax error is
the transport refusing to build a request at all.

The distinction is small and the reason to draw it is not. From
