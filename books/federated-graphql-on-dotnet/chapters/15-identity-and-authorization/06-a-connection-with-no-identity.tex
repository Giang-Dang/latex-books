...
requests. So the setting removes a fetch it can remove \emph{entirely}, and does
not prune a guarded field out of a fetch that has to happen anyway for a
permitted field beside it. Ask for the address on its own and Accounts is never
troubled; ask for the address and the display name together and the request goes
out carrying both, because the display name has to be fetched and the address is
on the same entity.
...
\index{authorization!counts that are not yet a gate}%
Which leaves a debt, and I would rather name it than let the table imply
otherwise. The counts in section~\ref{sec:ch15-a-token-at-the-router} are gate
steps, because a count that is the same on every machine belongs in a gate.
These three are not: nothing in the companion repository reproduces the
entity-fetch contrast yet. The two checks it wants are sketched in this
chapter's research note and they belong in \code{scripts/auth-run.mjs}, which
already counts Accounts requests and already holds the subscription pair. Until
they are there, this table is a measurement you can repeat by hand and not one
the build defends.
