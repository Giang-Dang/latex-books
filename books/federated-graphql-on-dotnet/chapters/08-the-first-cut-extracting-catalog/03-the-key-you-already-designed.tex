...
Rename the type in one of them and every representation stops matching, with no
composition error anywhere to warn you, because the schemas would still be
perfectly compatible. Figure~\ref{fig:ch08-key-round-trip} is the round trip.

\begin{figure}[htbp]
  \centering
  \input{figures/tikz/ch08-key-round-trip}
...
Here is the finding this section exists for. The federation package and the
global object identification feature do not know about each other at all. Grep
the whole federation package's source at tag \code{16.6.0} for \code{NodeId} or
for \code{Relay} and you get nothing back.
...
nullable precisely so that a subgraph can answer null for one entity without
failing the batch, which means nothing anywhere in the system considers this
worth reporting.
...
\code{ProductKey} exists twice, once in each service, byte for byte the same. I
considered a shared package and decided against it, and I would decide the same
way again: the moment the two services share a class, the format of the key
becomes a compile-time dependency between them, and the fact that they have to
\emph{agree about a string} disappears into a project reference. It is a
contract. Contracts get written down twice.
