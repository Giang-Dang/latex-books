...
\begin{enumerate}
  ...
  side. Nine fields between them, one key each, no overlap. Then find the two
  differences that are not fields: one type implements \code{Node} and the other
  does not, and only one of them has \code{ProductCategory} in its schema at all.
...
dotnet run --project src/Mosaic.Catalog -- \
  ...
  and compose again. Read the error. Then do the same for the two cost options,
  and again for the shareable page cursor class. Three settings, three errors,
  and none of the three is visible from inside either service.
...
Exercise~2 is the one to repeat. Every chapter from here to
chapter~\ref{ch:enter-the-router} adds machinery that does that assembly for
you, and the machinery is much easier to reason about once you have done it by
hand and know exactly how little of it is magic.
