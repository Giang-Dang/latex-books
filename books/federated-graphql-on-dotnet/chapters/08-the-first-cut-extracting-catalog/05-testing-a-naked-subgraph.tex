...
\code{averageRating} on \code{Product} and nothing else. Catalog publishes
\code{sku}, \code{title}, \code{description} and \code{category}. A field on
both sides is a composition error waiting for
chapter~\ref{ch:composition}; a field on neither is a client error waiting for
production.
...
Two representations worth sending on purpose. First, obvious rubbish:
...
A null, no \code{errors} key, HTTP 200. The second one is worth dwelling on: that
string is a real product identifier, and it is what this field answered before
...
nullable on purpose, so that one bad key in a batch of twenty-five does not cost
the other twenty-four their answers. A subgraph that throws instead turns a
single malformed representation into a failed fetch, and the router turns that
into a failed field, and the client gets nothing for a page of products because
one of them was wrong. Fail the entity, not the batch.

\subsection*{One call for twenty-five, and what happens when it is not}

\index{federation!representation batching!measured}%
Now the number I would put in front of anyone who thinks representation batching
...
is worth knowing and is not the same as saying it cost nothing. What it leaves
out is the round trip between the two services, which does not exist yet because
there is nothing between them.

\subsection*{Verify it in Postman}

\index{Postman!federation collection}%
Everything above is a request in the companion repository's collection, and the
collection is worth running rather than reading. Both files live in
\code{postman/}:
...
\index{Postman!assertions worth stealing}%
Two assertions in it are worth stealing for a graph of your own. One sends
Catalog three representations in a deliberately shuffled order and checks that
the three entities come back in that same shuffled order, because the contract
is positional and nothing in the response would tell you if a subgraph had
helpfully sorted them. The other asks Mosaic for \code{products} and asserts
that it fails, which is a strange-looking test until you consider what it
catches: a field quietly surviving on the wrong side of a cut is the failure
mode of every extraction, and it is invisible to a test that only checks that
