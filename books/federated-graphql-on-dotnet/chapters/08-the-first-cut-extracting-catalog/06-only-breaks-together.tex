...
to that one, because all three are settings on a subgraph, all three are on by
default, and all three describe something you cannot see from inside either
service. A subgraph that has never been composed is a subgraph whose schema
nobody has read.
...
subgraphs define unless one of them says it is shareable.
...
default, with the weight as a string, following the current draft of the cost
specification. The composer in wgc 0.129.7 carries a definition whose weight is
an \code{Int!}. Neither is wrong; they were written against different versions
of the same document, and the first thing that ever compares them is the
composer.
...
The pattern across all three is worth carrying forward. None of them was a
...
compare against. That is the real cost of the first cut: not the code that
moved, but the discovery that a schema you have been shipping for seven chapters
had opinions in it you never had to have.

Chapter~\ref{ch:composition} is where those opinions get read properly.
